export type Command = {
    ...
    cmd: 'set_reasoning_effort';
    sessionId: string;
    effort: ReasoningEffort;
} | {
    cmd: 'get_current_model';
    sessionId: string;
} | {
    cmd: 'list_models';
} | {
...
export type ToolCallStatus = 'pending-approval' | 'running' | 'success' | 'continued' | 'error' | 'denied' | 'cancelled' | (string & {});
...
export interface WireMessage {
    ...
    timestamp: number;
    parent_id: string | null;
    provenance: 'user' | 'assistant' | 'tool' | 'goal_continuation' | 'goal_objective_update';
...
export type ActorEvent = {
    ...
    type: 'plan_updated';
    sessionId: string;
    snapshot: PlanSnapshotWire | null;
} | {
    ...
    sessionId: string;
    snapshot: BackgroundTaskSnapshotWire;
} | {
    type: 'subagent_child';
    sessionId: string;
    id: string;
    event: SubagentChildWire;
} | {
...
export declare function isSessionEvent(ev: ActorEvent): ev is ActorEvent & {
    sessionId: string;
};
