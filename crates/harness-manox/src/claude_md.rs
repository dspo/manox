...
pub fn render_lazy(
    ...
    let data = agent::prompt::InstructionsPromptData {
        sources: sources
            .iter()
            .map(|s| agent::prompt::InstructionSourcePromptData {
                scope: s.kind.scope(),
    ...
    Some(
        agent::prompt::render(
            agent::prompt::PromptTemplate::WrapperInstructionsLazy,
            lang,
            &data,
        )
...
pub fn render_eager(set: &InstructionSet, lang: agent::language::Language) -> Option<String> {
    ...
    Some(
        agent::prompt::render(
            agent::prompt::PromptTemplate::WrapperInstructionsEager,
            lang,
            &data,
        )
...
mod tests {
    ...
    fn render_eager_none_when_empty() {
        ...
        assert!(render_eager(&set, agent::language::Language::En).is_none());
    }
    ...
    fn render_eager_tags_each_source_in_order() {
        ...
        let set = load(&t.0.join("proj"), &ctx_with(None, None));
        let text = render_eager(&set, agent::language::Language::En).expect("non-empty render");
        ...
        assert_eq!(
            text,
            render_eager(&set, agent::language::Language::En).unwrap()
        );
    }
    ...
    fn render_lazy_none_when_empty_and_reminder_when_present() {
        assert!(render_lazy(&[], agent::language::Language::En).is_none());
        ...
        let text = render_lazy(&found, agent::language::Language::En).expect("non-empty render");
        ...
        assert_eq!(
            text,
            render_lazy(&found, agent::language::Language::En).unwrap()
        );
    }
}
