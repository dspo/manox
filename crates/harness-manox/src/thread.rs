...
// agent-ui code that crosses the boundary sees one type.
pub use agent::thread::{ApprovalMode, SideCallMetric, ThreadId, ToolCallStatus};
...
pub enum ThreadEvent {
    ...
    ApprovalDecision {
        tool_name: String,
        tool_title: String,
        verdict: agent::approval::ReviewVerdict,
    },
    ...
    PlanUpdated {
        snapshot: agent::plan::PlanSnapshot,
    },
    ...
    BrowserNotification {
        tab_id: agent::webview_host::BrowserTabId,
        notification: agent::webview_host::BrowserNotification,
    },
    ...
    BackgroundTaskUpdated {
        snapshot: agent::background_task::TaskSnapshot,
    },
}
...
pub struct Thread {
    ...
    agent_language: agent::language::Language,
    ...
    background_tasks: Vec<agent::background_task::TaskSnapshot>,
    ...
    ui_notes: Vec<agent::db::UiNoteRecord>,
...
impl Drop for Thread {
    ...
    fn drop(&mut self) {
        ...
        if let Some(handle) = agent::runtime::try_handle() {
            let thread_id = self.id.0.clone();
            handle.spawn(async move {
                agent::background_task::cancel_all_for_thread(&thread_id).await;
                agent::background_task::remove_thread_mailbox(&thread_id);
            });
        }
        ...
        let Some(handle) = agent::runtime::try_handle() else {
            return;
        };
...
impl Thread {
    ...
    pub fn new(id: ThreadId, cwd: PathBuf, cx: &mut App) -> Entity<Self> {
        cx.new(|cx| {
            ...
            let agent_language = agent::settings::load().resolve().agent;
            Self {
                id,
                messages: Vec::new(),
                model,
    ...
    pub fn restore_with_goal(
        ...
        cx.new(|cx| {
            ...
            let agent_language = agent::language::Language::from_token(&rec.agent_language)
                .unwrap_or_else(|| {
            ...
            let background_tasks = rec
                ...
                .iter()
                .cloned()
                .map(agent::background_task::TaskSnapshot::normalize_after_restore)
                .collect();
            let restored_state = agent::compact::latest_compaction_state(&rec.messages);
    ...
    pub fn background_task_snapshots(&self) -> Vec<agent::background_task::TaskSnapshot> {
        let mut by_id: HashMap<String, agent::background_task::TaskSnapshot> = self
            .background_tasks
            .iter()
            .cloned()
            .map(|snapshot| (snapshot.task_id.clone(), snapshot))
            .collect();
        for snapshot in agent::background_task::snapshots_for_thread(&self.id.0) {
    ...
    /// mechanical summary. Mirrors [`agent::db::ThreadSummary::display_title`]
    ...
    pub fn deliver_peer_messages(
        ...
        for msg in &msgs {
            self.insert_user_message(
                agent::prompt::render(
                    agent::prompt::PromptTemplate::WrapperPeerMessage,
                    self.agent_language,
                    &agent::prompt::PeerMessageData {
                        from: msg.from.clone(),
    ...
    pub fn seed_approved_plan(
        &mut self,
        plan_text: String,
        ui: Option<agent::MessageUiMetadata>,
        cx: &mut Context<Self>,
    ) {
    ...
    fn cancel_goal_background_tasks(&self, goal_id: &str) {
        let Some(runtime) = agent::runtime::try_handle() else {
            tracing::warn!(
                goal_id,
        ...
        runtime.spawn(async move {
            agent::background_task::cancel_all_for_goal(&thread_id, &goal_id).await;
        });
    }
    ...
    pub(crate) fn flush_idle_goal_accounting(&mut self, cx: &mut Context<Self>) -> Result<()> {
        ...
        let goal = db.account_goal(
            &self.id.0,
            &goal_id,
            agent::goal::budget_tokens(delta),
    ...
    pub fn edit_goal(
        ...
        updated.objective = agent::goal::validate_objective(objective)?;
        agent::goal::validate_budget(token_budget)?;
    ...
    pub fn try_start_turn_if_idle(&mut self, cx: &mut Context<Self>) {
        ...
        let has_background_work = if agent::background_task::thread_has_pending_events(&self.id.0) {
        ...
        if agent::background_task::goal_has_running_tasks(&self.id.0, &goal.goal_id) {
    ...
    fn finish_goal_turn(&mut self, cancelled: bool, failed: bool, cx: &mut Context<Self>) {
        ...
        let db = crate::thread_store::global().read(cx).database();
        let token_delta = agent::goal::budget_tokens(delta);
        ...
        if self.goal().is_some_and(|goal| {
            goal.goal_id == goal_id
                ...
                && agent::background_task::goal_has_running_tasks(&self.id.0, &goal_id)
        }) {
            self.goal.turn_anchor = Some((
                goal_id.clone(),
                Instant::now(),
    ...
    fn enforce_goal_budget_during_turn(&mut self, cx: &mut Context<Self>) {
        ...
        if goal
            .tokens_used
            .saturating_add(agent::goal::budget_tokens(delta))
            < budget
        {
            return;
        }
    ...
    pub fn agent_language(&self) -> agent::language::Language {
        self.agent_language
    }
    ...
    pub fn set_archived(&mut self, archived: bool, cx: &mut Context<Self>) {
        ...
        } else if agent::background_task::thread_has_pending_events(&self.id.0) {
            agent::background_task::ensure_thread_mailbox(&self.id.0).notify_one();
        }
        cx.notify();
    }
    ...
    pub fn ui_notes(&self) -> &[agent::db::UiNoteRecord] {
        &self.ui_notes
    }
    ...
    pub fn set_ui_notes(&mut self, notes: Vec<agent::db::UiNoteRecord>) {
        self.ui_notes = notes;
    }
    ...
    pub fn push_ui_note(&mut self, note: agent::db::UiNoteRecord) {
    ...
    fn drain_background_events(&mut self, cx: &mut Context<Self>) -> bool {
        let events = agent::background_task::drain_thread_events(&self.id.0);
        if events.is_empty() {
            return false;
        }
        ...
        for event in &events {
            ...
            let rendered = agent::background_task::format_event_for_model(event);
        ...
        for tid in &task_ids {
            if let Some(task) = agent::background_task::get_by_str(tid) {
                let snapshot = task.snapshot(&agent::background_task::TaskId(tid.clone()));
    ...
    pub fn submit_skill(&mut self, key: &str, args: &str, cx: &mut Context<Self>) -> bool {
        ...
        let rendered = agent::prompt::render(
            agent::prompt::PromptTemplate::SkillBody,
            self.agent_language,
            &agent::prompt::SkillBodyData {
    ...
    pub fn run_turn(&mut self, cx: &mut Context<Self>) {
        ...
        if self.bg_watcher.is_none() {
            ...
            let notify = agent::background_task::ensure_thread_mailbox(&thread_id);
                    ...
                    let should_wake = this
                        .update(cx, |this, cx| {
                            if this.archived
                                ...
                                || !agent::background_task::thread_has_pending_events(&this.id.0)
                            {
                                return false;
                            }
                            let _ = cx;
    ...
    fn auto_compaction_target(&self) -> Option<usize> {
        ...
        let (max_input, threshold) = match model.auto_compact_window() {
            Some(window) => (window, agent::compact::CLAUDE_CODE_AUTO_COMPACT_THRESHOLD),
    ...
    pub fn compact(&mut self, _custom_instructions: Option<String>, cx: &mut Context<Self>) {
        ...
        let Some(insertion_ix) = agent::compact::forced_compaction_target_ix(&self.messages) else {
            return;
        };
    ...
    async fn compact_for_overflow_retry(
        ...
        let insertion_ix = this.read_with(cx, |this, _| {
            agent::compact::forced_compaction_target_ix(&this.messages)
        })?;
    ...
    async fn perform_compaction(
        ...
        let compaction_policy = crate::settings_ext::side_calls().compaction_policy();
        if !compaction_policy.enabled {
            anyhow::bail!("compaction side call is disabled");
        }
        let compaction_model = crate::settings_ext::side_call_model(&compaction_policy, model);
        let tokens_before = this
            .read_with(cx, |this, _| {
                this.token_meter
                    .per_request()
                    .values()
                    .map(|u| agent::compact::active_tokens(*u))
                    .max()
                    .unwrap_or(0)
            })
            .unwrap_or(0);
        ...
        let mut request = this.read_with(cx, |this, _| {
            ...
            let projected = if agent::settings::context_optimization().history_pruning
                == agent::settings::Toggle::On
            {
            ...
            crate::compaction_calls::build_compaction_request(
                &projected,
                projected.len(),
                this.agent_language,
            )
        ...
        let capsule = this.read_with(cx, |this, _| {
            let bound = insertion_ix.min(this.messages.len());
            let plan_steps =
                agent::plan::rebuild_from_messages(&this.messages[..bound]).map(|snapshot| {
                    snapshot
                        .steps
                        .into_iter()
                        .map(|step| agent::compact::PlanStepCapsule {
                            title: step.step,
            ...
            (
                ...
                active_tools,
                agent::compact::active_skills(&this.messages, bound),
                agent::compact::artifact_references(&this.messages, bound),
            )
        })?;
        let git_status = agent::compact::git_status_snapshot(capsule.0.clone()).await;
        ...
        let state =
            agent::compact::collect_compaction_state(agent::compact::CompactionStateInput {
                cwd: &capsule.0,
        ...
        this.update(cx, |this, cx| {
            ...
            let envelope = agent::compact::build_compaction_envelope(summary.clone(), state);
            let compaction_msg =
        ...
        if let Some(entity) = this.upgrade() {
            cx.update(|cx: &mut gpui::App| {
                ...
                store.update(cx, |s, cx| {
                    s.record_event(
                        &thread_id,
                        agent::db::ThreadEventType::Compaction,
                        &data,
                        cx,
                    );
                });
    ...
    async fn run_turn_loop(
        ...
        loop {
            ...
            if tool_uses.is_empty() {
                ...
                let recovery = this.update(cx, |this, cx| {
                    ...
                    if parse_err {
                        ...
                        this.insert_user_message(
                            agent::prompt::render(
                                agent::prompt::PromptTemplate::WrapperRecoveryFailure,
                                this.agent_language,
                                &agent::prompt::RecoveryFailureData {
                        ...
                        this.insert_user_message(
                            agent::prompt::render_static(
                                agent::prompt::PromptTemplate::WrapperUnfulfilledToolIntentNudge,
                                this.agent_language,
                            )
                        ...
                        this.insert_user_message(
                            agent::prompt::render(
                                agent::prompt::PromptTemplate::WrapperEmptyTurnNudge,
                                this.agent_language,
                                &agent::prompt::EmptyTurnNudgeData {}
                            )
                ...
                match recovery {
                    RecoveryAction::Done => {
                        ...
                        let has_bg_events = this.update(cx, |this, cx| {
                            if agent::background_task::thread_has_pending_events(&this.id.0) {
                                this.drain_background_events(cx)
                            } else {
                                false
                            }
            ...
            if !auto_review_tus.is_empty() {
                let review_enabled = crate::settings_ext::side_calls().approval_policy().enabled;
                let wk_root = this
                ...
                match model.as_ref() {
                    None => {
                        for tu in auto_review_tus {
                            ...
                            let verdict = agent::approval::ReviewVerdict::Ask {
                                reason: reason.clone(),
                            };
                            let _ = this.update(cx, |_, cx| {
                    ...
                    Some(model) => {
                        let review_items: Vec<agent::approval::ReviewItem> = auto_review_tus
                            .iter()
                            .map(|tu| agent::approval::ReviewItem {
                                id: tu.id.clone(),
                                tool_name: tu.name.to_string(),
                        ...
                        let review = crate::approval_review::review_batch(
                            model,
                            &review_items,
                            &cwd,
                            lang,
                        ...
                        for tu in auto_review_tus {
                            ...
                            let verdict = verdicts.get(&tu.id).cloned().unwrap_or(
                                agent::approval::ReviewVerdict::Ask {
            ...
            let hit_denial_breaker = this.update(cx, |this, cx| {
                ...
                if !this.denial_breaker_directive_injected {
                    ...
                    this.insert_user_message(
                        agent::prompt::render(
                            agent::prompt::PromptTemplate::WrapperDenialBreakerDirective,
                            this.agent_language,
                            &agent::prompt::DenialBreakerData {
            ...
            let hit_cap = this.update(cx, |this, cx| {
                ...
                if capped && !this.cap_summary_injected {
                    ...
                    this.insert_user_message(
                        agent::prompt::render(
                            agent::prompt::PromptTemplate::WrapperMaxTurnsSummary,
                            this.agent_language,
                            &agent::prompt::MaxTurnsSummaryData { max },
                        )
    ...
    async fn run_code_nested(
        ...
        let mut needs_approval = tool.requires_approval(&input)
            ...
            && !always_allowed;
        if needs_approval
            && crate::settings_ext::side_calls().approval_policy().enabled
            && let Some(model) = model.as_ref()
        {
            let started = std::time::Instant::now();
            let review = crate::approval_review::review(
                model,
                &name,
                &input,
                &title,
            ...
            match review.verdict {
                agent::approval::ReviewVerdict::Allow => needs_approval = false,
                agent::approval::ReviewVerdict::Ask { reason } => {
                    return fail(format!(
    ...
    async fn run_interactive_tool_inner(
        ...
        let (output, is_error) = match response {
            ToolAuthorizationResponse::AskUserQuestion { answers, response } => (
                agent::prompt::render(
                    agent::prompt::PromptTemplate::WrapperAskUserQuestions,
                    lang,
                    &agent::prompt::AskUserQuestionsData {
                        answers: answers
                            .into_iter()
                            .map(|(q, a)| agent::prompt::AskUserQa {
                                question: q,
                                answer: a,
                            })
                            .collect(),
            ...
            _ => {
                ...
                (
                    agent::prompt::render_static(
                        agent::prompt::PromptTemplate::WrapperToolDenied,
                        lang,
                    )
    ...
    async fn run_escalated_approval_inner(
        ...
        let question = agent::prompt::render(
            agent::prompt::PromptTemplate::WrapperEscalatedApprovalQuestion,
            lang,
            &agent::prompt::EscalatedApprovalQuestionData {
                tool_title: title.clone(),
                reason: reason.clone(),
            },
        )
        ...
        let allow_once_label = agent::i18n::t("workspace-escalation-allow-once");
        let always_allow_label = agent::i18n::t("workspace-escalation-always-allow");
        let deny_label = agent::i18n::t("workspace-escalation-deny");
        let input = serde_json::json!({
            "questions": [{
                "question": question,
                "header": agent::i18n::t("workspace-approval-title").to_string(),
                "multiSelect": false,
                "options": [
                    {
                        "label": allow_once_label.to_string(),
                        "description": agent::i18n::t("workspace-escalation-allow-once-desc").to_string(),
                    },
                    {
                        "label": always_allow_label.to_string(),
                        "description": agent::i18n::t("workspace-escalation-always-allow-desc").to_string(),
                    },
                    {
                        "label": deny_label.to_string(),
                        "description": agent::i18n::t("workspace-escalation-deny-desc").to_string(),
                    },
                ]
            }]
        });
    ...
    async fn run_tool_inner(
        ...
        if !is_error
            ...
            && let Ok(snapshot) = agent::plan::PlanSnapshot::from_input(&tu.input)
        {
            this.update(cx, |_, cx| {
    ...
    fn deny_tool_with_reason(
        ...
        let msg = format!(
            "{}\n\n{}",
            agent::prompt::render_static(agent::prompt::PromptTemplate::WrapperToolDenied, lang)
                .unwrap_or_default(),
            reason
        );
        this.update(cx, |this, cx| {
    ...
    fn maybe_augment_read_instructions(
        ...
        let ctx = crate::settings_ext::claude_md_load_context();
    ...
    fn append_max_tokens_directive(&mut self, _cx: &mut Context<Self>) {
        ...
        if let Some(m) = self.messages.last_mut() {
            m.push_content(MessageContent::Text(
                agent::prompt::render_static(
                    agent::prompt::PromptTemplate::WrapperMaxTokensDirective,
                    self.agent_language,
                )
    ...
    fn build_completion_request(&self) -> LanguageModelRequest {
        ...
        let capabilities = agent::prompt::ModelCapabilitiesPromptData {
        ...
        let system = agent::prompt::render(
            agent::prompt::PromptTemplate::SystemAssembly,
            self.agent_language,
            &agent::prompt::SystemPromptAssembly {
                base,
                ...
                worktree_subagent: if is_subagent {
                    self.worktree
                        .as_ref()
                        .map(|w| agent::prompt::WorktreePromptData {
        ...
        let history_pruning_on =
            agent::settings::context_optimization().history_pruning == agent::settings::Toggle::On;
        ...
        let mapped: Vec<LanguageModelRequestMessage> = crate::compaction_calls::coalesce_same_role(
            match agent::compact::latest_compaction_ix(
                &effective_messages,
                effective_messages.len(),
            ) {
                Some(c_ix) => {
                    let mut rebuilt = agent::compact::retained_user_messages_before(
                        &effective_messages,
                        c_ix,
                        self.agent_language,
                    );
        ...
        let messages = crate::compaction_calls::coalesce_same_role(messages);
...
pub(crate) fn model_facing_content(
    ...
    match c {
        ...
        MessageContent::ToolResult(tr)
            if agent::settings::context_optimization().effective_history_rewrite()
                == agent::settings::Toggle::On =>
        {
        ...
        MessageContent::Compaction(raw) => {
            ...
            let (mut summary, state) = agent::compact::parse_compaction(raw);
            ...
            MessageContent::Text(
                agent::prompt::render(
                    agent::prompt::PromptTemplate::WrapperCompactionPreamble,
                    lang,
                    &agent::prompt::CompactionPreambleData { summary },
                )
...
mod tests {
    ...
    fn model_facing_content_strips_agent_envelope_to_final() {
        ...
        let stripped = model_facing_content(&tr, agent::language::Language::En);
    ...
    fn model_facing_content_passes_non_agent_through() {
        ...
        let MessageContent::ToolResult(out) =
            model_facing_content(&tr, agent::language::Language::En)
        else {
            panic!("expected ToolResult");
        };
    ...
    fn model_facing_content_rewrites_compaction_to_text() {
        ...
        let facing = model_facing_content(&block, agent::language::Language::En);
    ...
    fn run_tool_inner_self_info_does_not_double_lease() {
        ...
        let thread = cx.update(|cx| {
            super::Thread::restore(
                agent::db::ThreadRecord::for_test("reg-run-tool-inner", "/tmp", Vec::new()),
                None,
                cx,
            )
        });
    ...
    fn cancel_clears_pending_authorizations() {
        ...
        let thread = cx.update(|cx| {
            super::Thread::restore(
                agent::db::ThreadRecord::for_test("reg-cancel-auth", "/tmp", Vec::new()),
                None,
                cx,
            )
        });
    ...
    fn restore_snapshots_agent_language_from_record() {
        crate::agent_def::init();
        let cx = gpui::TestAppContext::single();
        let mut rec = agent::db::ThreadRecord::for_test("reg-agent-language", "/tmp", Vec::new());
        rec.agent_language = "zh-CN".into();
        let thread = cx.update(|cx| super::Thread::restore(rec, None, cx));
        cx.update(|cx| {
            thread.read_with(cx, |t, _| {
                assert_eq!(t.agent_language(), agent::language::Language::ZhCn);
            });
        });
    }
    ...
    fn restore_defaults_agent_language_to_english() {
        ...
        let cx = gpui::TestAppContext::single();
        let rec =
            agent::db::ThreadRecord::for_test("reg-agent-language-default", "/tmp", Vec::new());
        let thread = cx.update(|cx| super::Thread::restore(rec, None, cx));
        cx.update(|cx| {
            thread.read_with(cx, |t, _| {
                assert_eq!(t.agent_language(), agent::language::Language::En);
            });
        });
    }
    ...
    fn restore_ignores_capsule_goal_and_rehydrates_tool_activations() {
        ...
        let envelope = agent::compact::build_compaction_envelope("handoff".into(), state);
        let messages = vec![Message::user_with_content(vec![
            MessageContent::Compaction(envelope),
        ])];
        let thread = cx.update(|cx| {
            super::Thread::restore(
                agent::db::ThreadRecord::for_test("restore-capsule", "/tmp", messages),
                None,
                cx,
            )
        });
    ...
    fn code_nested_write_denied_without_model() {
        ...
        let thread = cx.update(|cx| {
            super::Thread::restore(
                agent::db::ThreadRecord::for_test("code-approval", "/tmp", Vec::new()),
                None,
                cx,
            )
        });
    ...
    fn interactive_tool_bypasses_permission_cache_and_renders_answers() {
        ...
        let thread = cx.update(|cx| {
            super::Thread::restore(
                agent::db::ThreadRecord::for_test("ask-interactive", "/tmp", Vec::new()),
                None,
                cx,
            )
        });
    ...
    fn cancel_emits_explicit_cancelled_stop_reason() {
        ...
        let thread = cx.update(|cx| {
            super::Thread::restore(
                agent::db::ThreadRecord::for_test("reg-cancel-reason", "/tmp", Vec::new()),
                None,
                cx,
            )
        });
    ...
    fn leader_cancel_propagates_to_team_members() {
        ...
        let (leader, member, team, member_token) = cx.update(|cx| {
            let leader = super::Thread::restore(
                agent::db::ThreadRecord::for_test("lead", "/tmp", Vec::new()),
                None,
                cx,
            );
            let mut member_rec = agent::db::ThreadRecord::for_test("plan", "/tmp", Vec::new());
            member_rec.depth = 1;
    ...
    #[test]
    fn build_completion_request_emits_effort_directly() {
        ...
        let thread = cx.update(|cx| {
            super::Thread::restore(
                agent::db::ThreadRecord::for_test("reg-effort", "/tmp", Vec::new()),
                None,
                cx,
            )
        });

        // High → High on the wire.
    ...
    fn build_completion_request_injects_instructions_at_fixed_slot() {
        ...
        let thread = cx.update(|cx| {
            super::Thread::restore(
                agent::db::ThreadRecord::for_test("claude-md-slot", &cwd, Vec::new()),
                None,
                cx,
            )
        });
    ...
    fn build_completion_request_injector_slot_is_byte_stable() {
        ...
        let thread = cx.update(|cx| {
            super::Thread::restore(
                agent::db::ThreadRecord::for_test("turn-ext-equiv", &cwd, Vec::new()),
                None,
                cx,
            )
        });
        ...
        if let Some(text) = crate::claude_md::render_eager(
            &crate::claude_md::load(
                ...
                &crate::settings_ext::claude_md_load_context(),
            ),
            lang,
        ) {
    ...
    fn read_file_tool_result_injects_nested_instructions_once() {
        ...
        let thread = cx.update(|cx| {
            super::Thread::restore(
                agent::db::ThreadRecord::for_test("lazy-inject", &cwd, Vec::new()),
                None,
                cx,
            )
        });
        ...
        let thread2 = cx.update(|cx| {
            super::Thread::restore(
                agent::db::ThreadRecord::for_test("lazy-inject-off", &cwd, Vec::new()),
                None,
                cx,
            )
        });
    ...
    fn build_completion_request_gates_tools_and_images_on_capability() {
        ...
        let thread = cx.update(|cx| {
            super::Thread::restore(
                agent::db::ThreadRecord::for_test("reg-cap-no-tools", "/tmp", Vec::new()),
                Some(no_tools),
                cx,
            )
        });
        ...
        let thread = cx.update(|cx| {
            super::Thread::restore(
                agent::db::ThreadRecord::for_test("reg-cap-able", "/tmp", Vec::new()),
                Some(capable),
                cx,
            )
        });
    ...
    fn request_projection_never_mutates_canonical_or_persisted_messages() {
        ...
        let thread = cx.update(|cx| {
            super::Thread::restore(
                agent::db::ThreadRecord::for_test(
    ...
    fn build_completion_request_no_adjacent_same_role_after_mode_injection() {
        ...
        let thread = cx.update(|cx| {
            super::Thread::restore(
                agent::db::ThreadRecord::for_test("reg-no-adjacent-role", "/tmp", Vec::new()),
                Some(capable),
                cx,
            )
        });
    ...
    fn run_tool_inner_danger_ask_user_question_hits_gate_not_run() {
        ...
        let thread = cx.update(|cx| {
            let mut rec =
                agent::db::ThreadRecord::for_test("reg-danger-ask-user", "/tmp", Vec::new());
    ...
    async fn live_run_turn_drains_full_stream() {
        ...
        let thread = cx.update(|cx| {
            let mut rec = agent::db::ThreadRecord::for_test(
                "live-run-turn",
    ...
    fn goal_runs_through_explicit_complete_and_accounts_final_response() {
        ...
        let db = Arc::new(
            agent::db::ThreadsDatabase::open(std::path::Path::new(":memory:"))
    ...
    fn goal_waits_for_owned_background_event_before_continuing() {
        ...
        let (task_id, task) = cx.update(|cx| {
            thread.update(cx, |thread, cx| {
                ...
                let registered = agent::background_task::register_for_goal(
                    agent::background_task::TaskKind::MonitorCommand,
                    thread_id.into(),
                    "goal monitor".into(),
        ...
        cx.update(|cx| {
            thread.read_with(cx, |thread, _| {
                ...
                let continuation_ix = thread
                    ...
                    .iter()
                    .position(|message| {
                        message.provenance == agent::MessageProvenance::GoalContinuation
                    })
        ...
        agent::background_task::remove(&task_id);
        agent::background_task::remove_thread_mailbox(thread_id);
    }
    ...
    fn replacement_fences_old_reservation_and_no_model_does_not_reserve() {
        ...
        let old_task_id = cx.update(|cx| {
            thread.update(cx, |thread, cx| {
                ...
                let (old_task_id, old_task) = agent::background_task::register_for_goal(
                    agent::background_task::TaskKind::MonitorCommand,
                    thread.id.0.clone(),
        ...
        cx.run_until_parked();
        agent::background_task::remove(&old_task_id);
        agent::background_task::remove_thread_mailbox("goal-replacement-fence");
    }
    ...
    #[cfg(test)]
    fn count_nudges(messages: &[agent::message::Message]) -> usize {
        messages
            .iter()
    ...
    #[cfg(test)]
    fn count_compactions(messages: &[agent::message::Message]) -> usize {
        messages
            .iter()
            .filter(|m| {
                m.content
    ...
    fn failed_compaction_preserves_history_and_previous_capsule() {
        ...
        let messages = vec![
            ...
            Message::user_with_content(vec![MessageContent::Compaction(
                agent::compact::build_compaction_envelope("previous summary".into(), state),
            )]),
        ...
        let restored = agent::compact::latest_compaction_state(
    ...
    fn enqueue_steer_ids_and_cancel_pending_steer() {
        ...
        let thread = cx.update(|cx| {
            super::Thread::restore(
                agent::db::ThreadRecord::for_test("reg-steer-ids", "/tmp", Vec::new()),
                None,
                cx,
            )
        });
    ...
    fn drain_pending_steer_tags_drained_message_steered() {
        ...
        let thread = cx.update(|cx| {
            super::Thread::restore(
                agent::db::ThreadRecord::for_test("reg-steer-drain", "/tmp", Vec::new()),
                None,
                cx,
            )
        });
    ...
    fn danger_mode_runs_approval_required_tool() {
        ...
        cx.update(agent::runtime::init);
        let thread = cx.update(|cx| {
            let mut rec = agent::db::ThreadRecord::for_test("danger-gate", "/tmp", Vec::new());
    ...
    fn fallback_gate_denial_carries_reason() {
        ...
        let thread = cx.update(|cx| {
            super::Thread::restore(
                agent::db::ThreadRecord::for_test("fallback-gate", "/tmp", Vec::new()),
                None,
                cx,
            )
        });
    ...
    fn denial_streak_counts_and_resets() {
        ...
        cx.update(agent::runtime::init);
        let thread = cx.update(|cx| {
            let mut rec = agent::db::ThreadRecord::for_test("denial-streak", "/tmp", Vec::new());
            rec.approval_mode = super::ApprovalMode::Danger.as_i64();
            super::Thread::restore(rec, None, cx)
        });
        let weak = thread.downgrade();
    ...
    fn always_allow_response_plants_grant() {
        ...
        let thread = cx.update(|cx| {
            super::Thread::restore(
                agent::db::ThreadRecord::for_test("always-allow", "/tmp", Vec::new()),
                None,
                cx,
            )
        });
    ...
    fn reviewer_allowed_call_passes_fallback_gate() {
        ...
        let thread = cx.update(|cx| {
            super::Thread::restore(
                agent::db::ThreadRecord::for_test("reviewer-allow", "/tmp", Vec::new()),
                None,
                cx,
            )
        });
    ...
    fn tool_gate_deny_blocks_reviewer_allowed_call() {
        ...
        let thread = cx.update(|cx| {
            super::Thread::restore(
                agent::db::ThreadRecord::for_test("gate-deny", "/tmp", Vec::new()),
                None,
                cx,
            )
        });
    ...
    fn escalated_ask_allow_once_runs_the_call() {
        ...
        let thread = cx.update(|cx| {
            super::Thread::restore(
                agent::db::ThreadRecord::for_test("escalated-allow", "/tmp", Vec::new()),
                None,
                cx,
            )
        });
    ...
    fn escalated_ask_always_allow_plants_grant_and_runs() {
        ...
        let thread = cx.update(|cx| {
            super::Thread::restore(
                agent::db::ThreadRecord::for_test("escalated-allow-all", "/tmp", Vec::new()),
                None,
                cx,
            )
        });
    ...
    fn escalated_ask_resolves_option_label_answer() {
        ...
        let thread = cx.update(|cx| {
            super::Thread::restore(
                agent::db::ThreadRecord::for_test("escalated-label", "/tmp", Vec::new()),
                None,
                cx,
            )
        });
    ...
    fn escalated_ask_always_allow_label_round_trip() {
        ...
        let thread = cx.update(|cx| {
            super::Thread::restore(
                agent::db::ThreadRecord::for_test("escalated-always-label", "/tmp", Vec::new()),
                None,
                cx,
            )
        });
    ...
    fn escalated_ask_denied_feed_back_and_streak() {
        ...
        let thread = cx.update(|cx| {
            super::Thread::restore(
                agent::db::ThreadRecord::for_test("escalated-deny", "/tmp", Vec::new()),
                None,
                cx,
            )
        });
