//! The manox harness tool implementations plus the default registry assembly.
//!
//! The tool-name constants live in `::agent::tools` (shared with the pi path's
//! consumers) and are re-exported here.
...
pub(crate) fn base_tools(cwd: Arc<PathBuf>) -> Vec<AnyAgentTool> {
    ...
    base_tools_with_policy(cwd, sandbox, None, ::agent::language::Language::En)
}
...
pub(crate) fn base_tools_with_policy(
    ...
    if matches!(
        ::agent::settings::context_optimization().code_mode,
        ::agent::settings::CodeModeToggle::Hybrid
    ) {
...
pub fn main_registry_with_policy(
    ...
    if matches!(
        ::agent::settings::context_optimization().tool_discovery,
...
mod tests {
    ...
    fn update_plan_is_main_only() {
        ...
        let full_names: Vec<String> = reg
            .to_request_tools(::agent::language::Language::En)
            .into_iter()
            .map(|t| t.name)
            .collect();
        assert!(
