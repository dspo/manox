...
use agent::background_task::TaskId;
...
pub async fn spawn(
    ...
    let (task_id, bg_task) = agent::background_task::register_for_goal(
        agent::background_task::TaskKind::BackgroundBash,
        thread_id.into(),
        command.clone(),
        bg_cancel.clone(),
        goal_id,
        ...
        .await
    {
        ...
        Err(e) => {
            ...
            bg_task.push_terminal(&task_id, agent::background_task::TaskStatus::Failed);
    ...
    register_bash_shell(&shell_id, shell_state.clone());
    ...
    let driver = async move {
        ...
        let (terminal_status, code) = tokio::select! {
            code = process.wait_for_exit() => {
                ...
                // misreporting Failed.
                let stop_in_flight =
                    bg_task_clone.status() == agent::background_task::TaskStatus::Stopping;
                (
                    if code == Some(0) {
                        agent::background_task::TaskStatus::Completed
                    } else if stop_in_flight {
                        bg_task_clone.requested_stop_status()
                    } else {
                        agent::background_task::TaskStatus::Failed
                    },
                    code,
                )
            }
            ...
            _ = async {
                ...
                process.cleanup_process_group_after_exit().await;
                (
                    agent::background_task::TaskStatus::TimedOut,
                    code,
                )
            }
        };
    ...
    let driver_handle = if let Some(h) = agent::runtime::try_handle() {
        h.spawn(driver)
    } else {
        tokio::spawn(driver)
    };
...
async fn drain_stream<R: tokio::io::AsyncRead + Unpin>(
    ...
    label: &str,
    task: Arc<agent::background_task::BackgroundTask>,
    task_id: TaskId,
) {
...
pub fn snapshots(thread_id: &str) -> Vec<String> {
    let mut snapshots = agent::background_task::snapshots_for_thread(thread_id)
        .into_iter()
        .filter(|snapshot| snapshot.kind == agent::background_task::TaskKind::BackgroundBash)
        .map(|snapshot| {
            format!(
                "{}:{}:{}B",
                snapshot.task_id,
...
mod tests {
    ...
    async fn nonzero_exit_is_failed_and_terminal_is_last() {
        ...
        let task = agent::background_task::get_by_str(&id).expect("registered task");
        for _ in 0..100 {
            if task.status() != agent::background_task::TaskStatus::Running {
                break;
            }
            tokio::time::sleep(Duration::from_millis(50)).await;
        }
        assert_eq!(task.status(), agent::background_task::TaskStatus::Failed);
        assert_eq!(task.exit_code(), Some(7));
        let events = agent::background_task::drain_thread_events("test-thread-failed");
        assert!(matches!(
            events.last().map(|event| &event.event),
            Some(agent::background_task::TaskEventKind::Terminal {
        ...
        assert!(events.iter().any(|event| {
            matches!(&event.event, agent::background_task::TaskEventKind::Output(text) if text.contains("tail"))
        }));
    }
