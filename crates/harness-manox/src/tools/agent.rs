...
/// descriptions.
fn build_description(lang: agent::language::Language) -> Arc<str> {
    let subagents: Vec<agent::prompt::SubagentTypeData> = agent_def::global()
        .entries()
        .into_iter()
        .map(|(key, d)| agent::prompt::SubagentTypeData {
            name: key.clone(),
            capability: capability_tag(&d.def),
            description: d.def.description.clone(),
        })
        .collect();
    let s = agent::prompt::render(
        agent::prompt::PromptTemplate::AgentToolDescription,
        lang,
        &agent::prompt::AgentToolDescriptionData { subagents },
    )
