    ...
    fn inject(&self, ctx: &TurnCtx) -> Vec<LanguageModelRequestMessage> {
        if ctx.instructions_enabled
            && let Some(text) = crate::claude_md::render_eager(
                &crate::claude_md::load(ctx.cwd, &crate::settings_ext::claude_md_load_context()),
                ctx.agent_language,
            )
        {
...
impl CompactionPolicy for DefaultCompactionPolicy {
    fn decide(&self, input: &CompactionInput) -> CompactionOutcome {
        let Some(target) = agent::compact::auto_compaction_target_ix(
            input.messages,
            input.per_request,
            true,
        ...
        if agent::settings::context_optimization().history_pruning == agent::settings::Toggle::On {
                ...
                crate::retention::prune_for_compaction(prefix, input.cwd, input.thread_id)
            {
                ...
                if agent::compact::effective_context_tokens(
                    &pruned,
