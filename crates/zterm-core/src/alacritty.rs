...
use crate::{
    AlternateScroll, Cell, Color, Content, Cursor, CursorShape, GridLinesChange, HoveredWord,
    Hyperlink, HyperlinkData, IndexedCell, Modes, Point, PtyEvent, Range, RenderableCells, Scroll,
...
fn alacritty_cursor_shape(cursor_shape: CursorShape) -> AlacCursorShape {
    match cursor_shape {
        ...
        CursorShape::Hidden => AlacCursorShape::Block,
    }
}
...
mod tests {
    ...
    fn semantic_selection_stops_at_tree_branch() {
        let config = pty_term_config(1000, CursorShape::default());
