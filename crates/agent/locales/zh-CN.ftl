...
settings-desc-follow-up = 在 manox 运行的同时将后续操作加入队列，或引导当前运行。按下"⌘ + ⌥ + /"可对单条消息执行撤销操作
...
workspace-plan-approval-title = 已提交计划，待审批
workspace-plan-approval-question = 批准将退出 plan 模式并开始执行；拒绝将结束本轮并保持在 plan 模式——你下一条消息即新的方向。
workspace-plan-continue = 拒绝并结束本轮
