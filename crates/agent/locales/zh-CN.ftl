...
context-opt-title = Tokens 消费分布
context-opt-heading-projection = 投影
context-opt-label-sent = 已发送
context-opt-label-baseline = 基线
context-opt-label-saved = 节省
context-opt-heading-breakdown = 分布
context-opt-label-system = 系统
context-opt-label-mode = 模式
context-opt-label-project = 项目
context-opt-label-schemas = 工具定义
context-opt-label-history = 历史
context-opt-label-results = 结果
context-opt-heading-tools = 工具优化
context-opt-label-tool-schemas = 活跃/总数
context-opt-label-rewrite = 重写
context-opt-label-pruning = 剪枝
context-opt-label-discovery = 发现
context-opt-heading-runtime = 运行时
context-opt-label-prefix = 前缀缓存
context-opt-label-avoided-compact = 避免压缩
