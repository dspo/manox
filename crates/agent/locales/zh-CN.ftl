...
settings-models-file-caption = Provider 配置文件：{$path}
settings-models-no-path = 无法解析 Provider 配置路径
settings-models-reload = 重新读取
settings-models-add-provider = 添加 Provider
settings-models-save = 保存
settings-models-saved = Provider 配置已保存
...
settings-models-count = {$models} 个模型 · {$endpoints} 个端点
settings-models-load-error-title = 配置读取失败
settings-models-load-error-hint = 为避免覆盖无法识别的文件，保存已禁用。请修正文件或重新读取。
settings-models-row-name = 名称
settings-models-ph-name = Provider 显示名称
settings-models-row-apikey = API Key 来源
settings-models-ph-apikey = keychain:SERVICE · env:VAR · literal:VALUE · $(shell 命令)
settings-models-section-endpoints = 端点
settings-models-row-url = URL
settings-models-ph-url = https://api.example.com
settings-models-row-agents = 可见 Agent
settings-models-ph-agents = 逗号分隔，留空 = 全部 Agent
settings-models-value-unset = 未设置
settings-models-add-endpoint = 添加端点
settings-models-section-env = 环境变量
settings-models-add-env = 添加变量
settings-models-section-models = 模型
settings-models-mode-inline = 内联定义
settings-models-mode-remote = 远程 URL
...
settings-models-row-wire-apis = 传输协议
