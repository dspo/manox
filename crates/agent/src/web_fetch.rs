...
//! behind auth or rendered by client-side JS, use the `web_explore_*` tools
//! instead.
...
impl AgentTool for WebFetchTool {
    ...
    fn parameters_schema(&self) -> serde_json::Value {
        let mut value = serde_json::to_value(schemars::schema_for!(WebFetchInput)).expect("schema");
    ...
    async fn execute(
        ...
        let input: WebFetchInput = serde_json::from_value(params)
            .map_err(|e| ToolError::InvalidArguments(format!("input parse failed: {e}")))?;
        let max_bytes = input
            .max_bytes
            .unwrap_or(DEFAULT_MAX_BYTES)
            .clamp(1, MAX_ALLOWED_BYTES);
        let timeout_secs = input.timeout_secs.unwrap_or(DEFAULT_TIMEOUT_SECS);
        fetch(input.url, max_bytes, timeout_secs, signal)
            .await
...
async fn fetch(
    ...
    let advisory = if truncated || total_received > max_bytes {
...
mod tests {
    ...
    async fn fetch_rejects_non_http() {
        // Synchronous prefix check happens before any network IO.
        let cancel = CancellationToken::new();
        let r = fetch("file:///etc/passwd".into(), 1024, 5, cancel)
            .await
            .unwrap_err();
    ...
    async fn fetch_rejects_invalid_url() {
        let cancel = CancellationToken::new();
        let r = fetch("https://".into(), 1024, 5, cancel).await.unwrap_err();
        assert!(r.contains("WebFetch"), "{r}");
    }

    #[test]
    fn output_header_format() {
        // Header block shape is a contract; pin it.
        let s = format!(
            "URL: {}\nStatus: {}\nContent-Type: {}\nBytes: {}\n\nbody",
