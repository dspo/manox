...
- **`TeamCreate`** — a peer team of long-lived worker members (you are the leader) for sub-tasks that need to coordinate, share progress, or have runtime dependencies between them. Members run autonomously and report back via `SendMessage`; everyone reads and updates the shared `Task*` list. Use `SendMessage` to hand off work, report progress, or ask a clarifying question mid-flight. Only form a team for genuinely parallel, non-trivial work — N members means N× tokens and rate-limit pressure.
