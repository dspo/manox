...
//! column. When a symbol occurs twice on one line, the optional 1-indexed
//! column disambiguates it. Code-intel calls go through
//! `LspRegistry::client_for_path`, which ensures + bounded-waits (≤5s) and
//! returns an "indexing, retry" error instead of blocking forever.
...
/// Flat schemars schema (providers reject `$ref`/`$defs`).
...
/// Resolve a tool path against the session cwd (relative paths).
...
/// Cap `text` at `max` bytes on a char boundary; when anything was cut,
/// append an elision marker plus `hint`.
fn truncate_output(text: &str, max: usize, hint: &str) -> String {
    if text.len() <= max {
...
fn render_locs(locs: Vec<(PathBuf, u32, u32)>, empty_hint: &str, hint: &str) -> String {
    ...
    let body = locs
        ...
        .collect::<Vec<_>>()
        .join("\n");
    truncate_output(&body, OUTPUT_CAP, hint)
}
...
impl AgentTool for GoToDefinitionTool {
    ...
    async fn execute(
        ...
        let Ok(parsed) = serde_json::from_value::<PositionInput>(params) else {
            return Err(ToolError::InvalidArguments(
                "input parse failed".to_string(),
            ));
        };
...
impl AgentTool for FindReferencesTool {
    ...
    async fn execute(
        ...
        let Ok(parsed) = serde_json::from_value::<ReferencesInput>(params) else {
            return Err(ToolError::InvalidArguments(
                "input parse failed".to_string(),
            ));
        };
    ...
    async fn execute(
        ...
        let result: anyhow::Result<String> = async move {
            ...
            match client
                ...
                .await?
            {
                Some(text) => Ok(truncate_output(&text, OUTPUT_CAP, "narrow the symbol")),
...
impl AgentTool for DocumentSymbolsTool {
    ...
    async fn execute(
        ...
        let Ok(parsed) = serde_json::from_value::<PathInput>(params) else {
            return Err(ToolError::InvalidArguments(
                "input parse failed".to_string(),
            ));
        };
        ...
        let result: anyhow::Result<String> = async move {
            ...
            Ok(truncate_output(
                &body,
                OUTPUT_CAP,
                "scope to a region or use a narrower file",
            ))
        }
        .await;
        match result {
...
impl AgentTool for WorkspaceSymbolsTool {
    ...
    async fn execute(
        ...
        let Ok(parsed) = serde_json::from_value::<QueryInput>(params) else {
            return Err(ToolError::InvalidArguments(
                "input parse failed".to_string(),
            ));
        };
        ...
        let result: anyhow::Result<String> = async move {
            ...
            let body = all
                ...
                .collect::<Vec<_>>()
                .join("\n");
            Ok(truncate_output(&body, OUTPUT_CAP, "narrow the query"))
        }
        .await;
        match result {
            Ok(text) => Ok(AgentToolResult::text(text)),
...
impl AgentTool for DiagnosticsTool {
    ...
    async fn execute(
        ...
        let Ok(parsed) = serde_json::from_value::<DiagnosticsInput>(params) else {
            return Err(ToolError::InvalidArguments(
                "input parse failed".to_string(),
            ));
        };
        ...
        let result: anyhow::Result<String> = async move {
            ...
            Ok(truncate_output(
                &bodies.join("\n\n"),
                OUTPUT_CAP,
                "scope to a single file via the `path` argument",
            ))
        }
        .await;
        match result {
...
/// Wait for the registry probe to land (the caller bounds the wait).
