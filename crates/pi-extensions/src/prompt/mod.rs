...
//! callers always receive a [`pi::harness::SystemPromptBuilder`].
//!
...
fn assembly_data<'a>(
    ...
    cwd: &std::path::Path,
    active_tools: &[String],
    resources: &pi::harness::HarnessResources,
) -> AssemblyData<'a> {
    AssemblyData {
        base,
        ...
        context_files: resources
            ...
            .iter()
            .map(|f| CtxFileData {
                location: pi::system_prompt::xml_escape(&f.location),
        ...
        skills: resources
            ...
            .iter()
            .map(|s| AdvSkillData {
                name: pi::system_prompt::xml_escape(&s.name),
                description: pi::system_prompt::xml_escape(&s.description),
                location: pi::system_prompt::xml_escape(&s.location),
            })
            .collect(),
    }
}
...
/// The Captain session's [`pi::harness::SystemPromptBuilder`]: the Captain
/// base is rendered once from the config; the fold re-renders on every
/// active-tool/resource rebuild.
pub fn captain_prompt_builder(config: CaptainConfig) -> pi::harness::SystemPromptBuilder {
    let base = render(
        CAPTAIN_KEY,
        &CaptainData {
    ...
    let cwd = config.cwd;
    std::sync::Arc::new(
        move |active_tools: &[String], resources: &pi::harness::HarnessResources| {
            render_assembly(&base, &cwd, active_tools, resources)
        },
    )
}

/// A [`pi::harness::SystemPromptBuilder`] for sessions that bring their own
...
pub fn base_prompt_builder(base: String, cwd: PathBuf) -> pi::harness::SystemPromptBuilder {
    std::sync::Arc::new(
        move |active_tools: &[String], resources: &pi::harness::HarnessResources| {
            render_assembly(&base, &cwd, active_tools, resources)
        },
