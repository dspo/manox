...
pub fn normalize(text: &str) -> String {
    ...
    // split('\n') yields a trailing empty segment when the text ends in '\n';
    // the pop removes the synthetic newline appended for that segment, so the
    // normalized form keeps exactly the original's trailing-newline status.
    if out.ends_with('\n') {
        out.pop();
    }
    out
