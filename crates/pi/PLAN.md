...
**基线（S0 冻结，S8–S10 复核轮确认）：** Rust HEAD `（S14 待提交）`（S8–S14 复核轮修复已提交）；TS Pi 冻结 `4488ad55c18f07ae89a489096c90de8667b3adfb`（upstream 已推进至 `ebf33c0c2`，新增 delta 仅 UI/fullscreen copy confirmation 类型，无 agreed non-UI 漂移）。
