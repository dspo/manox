...
cd "$ROOT_DIR"
cargo build --release
# crates/cx is a workspace member of manox, so release artifacts land in the
# workspace root's target directory; resolve it instead of assuming a local one.
TARGET_DIR="$(cargo metadata --format-version 1 --no-deps | python3 -c 'import json,sys; print(json.load(sys.stdin)["target_directory"])')"
install "$TARGET_DIR/release/cx" "$TARGET_BIN"
