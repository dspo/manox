...
impl Thread {
    ...
    fn handle_notice_inner(&mut self, notice: BackendNotice) {
        match notice {
            ...
            BackendNotice::Ready(notice) => {
                ...
                if plan_review_pending && let Some(plan_file) = plan_file {
    ...
    pub fn set_goal_status(
        ...
        if status == crate::goal::GoalStatus::Active
            && !was_active
...
impl Thread {
    ...
    fn run_goal_slash(&mut self, args: &str, ui: Option<MessageUiMetadata>) {
        ...
        if let Some(objective) = trimmed.strip_prefix("edit ").map(str::trim) {
            ...
            if let Err(error) = self.edit_goal(
                objective.to_string(),
                budget,
                max_rounds,
                crate::goal::GoalActor::User,
    ...
    fn user_prompt_ordinal(&self) -> usize {
        self.messages
            .iter()
            .filter(|m| {
                m.role == Role::User && m.provenance == crate::message::MessageProvenance::User
            })
            .count()
    }
...
pub(crate) mod tests {
    ...
    fn registry_display_persistence_dispatches_off_runtime() {
        ...
        let result = std::panic::catch_unwind(|| {
            persist_registry_display_spawn(sessions_dir.clone(), session_path.clone(), 0, display)
        });
    ...
    async fn sailor_completed_notice_injects_peer_message_and_fires_turn() {
        ...
        assert!(
            runs[0].0.contains("PR #601 LGTM"),
            "final text rode the prompt: {:?}",
            runs[0].0
        );
        drop(runs);
