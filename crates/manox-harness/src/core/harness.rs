...
fn is_transient_error(err: &anyhow::Error) -> bool {
    if let Some(pe) = err.downcast_ref::<crate::core::provider::ProviderError>() {
        return match pe {
            crate::core::provider::ProviderError::Http { status, body } => {
                crate::core::provider::retry::is_retryable_status(*status)
                    && !is_quota_or_billing(body)
            }
            crate::core::provider::ProviderError::Transport(_) => true,
            _ => false,
        };
    }
    false
...
impl<S: SessionStorage + 'static> AgentHarness<S> {
    ...
    pub fn request_observer(&self) -> Arc<dyn crate::core::provider::RequestObserver> {
        struct Observer(Arc<Mutex<Vec<(HookPoint, HookHandler)>>>);
        impl crate::core::provider::RequestObserver for Observer {
            fn before_payload(
                &self,
                attempt: u32,
    ...
    pub(crate) fn run_input_hook(
        ...
        for (point, handler) in hooks.iter() {
            ...
            ctx.data = serde_json::to_value(event).unwrap_or(serde_json::Value::Null);
            ctx = handler(ctx);
            if ctx.input_handled {
                return ctx;
            }
            if let Some(transform) = ctx.input_transform.take() {
                current_text = transform.text;
                if let Some(images) = transform.images {
        ...
        if transformed {
            ctx.input_transform = Some(InputHookTransform {
                text: current_text,
                images: Some(current_images),
            });
        }
        ctx
    }
    ...
    async fn prepare_auto_retry(&mut self) -> Result<bool, anyhow::Error> {
        ...
        if crate::core::provider::overflow::is_context_overflow(
            message,
...
pub(crate) mod tests {
    ...
    fn summarization_retry_classifies_errors() {
        let transient = anyhow::anyhow!(crate::core::provider::ProviderError::Http {
            status: 503,
            body: "unavailable".into(),
        });
        assert!(is_transient_error(&transient));
        let transport = anyhow::anyhow!(crate::core::provider::ProviderError::Transport(
            "connection reset".into()
        ));
        assert!(is_transient_error(&transport));
        let auth = anyhow::anyhow!(crate::core::provider::ProviderError::Http {
            status: 401,
            body: "unauthorized".into(),
        });
        assert!(!is_transient_error(&auth));
        let quota = anyhow::anyhow!(crate::core::provider::ProviderError::Http {
            status: 429,
        ...
        let rate_limit = anyhow::anyhow!(crate::core::provider::ProviderError::Http {
            status: 429,
