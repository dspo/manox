...
// (`:N-` means "from N to EOF"). Same shape as the kernel's hashline range.
pub use crate::core::hashline::LineRange;
...
fn try_parse_ranges(sel: &str) -> Option<Vec<LineRange>> {
    ...
    let mut merged: Vec<LineRange> = vec![parsed[0]]; // LineRange is Copy
...
fn parse_line_range_chunk(chunk: &str) -> Option<LineRange> {
    ...
    match sep {
        ...
        Some('+') => {
            ...
            // Saturating matches the kernel Read's paging posture — a
            // pathological count clamps to EOF instead of overflowing.
            Some(LineRange {
                start,
