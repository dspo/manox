// End-to-end smoke check for the OpenAI Responses provider against a live
// endpoint.
//
// Usage:
...
//   --model qwen3-max --thinking-kind adaptive --thinking low
//
...
//   protocol, mapped onto Model.thinking. Both "enabled" and "adaptive" encode
//   as reasoning:{effort} with summary:"auto" and encrypted_content included.
// --thinking: off | minimal | low | medium | high | xhigh | max — the harness
//   thinking level. "off" (or a thinking kind with no level) degrades to
//   reasoning:{effort:"none"}; omitting it sends no reasoning field at all.

use manox_harness::core::provider::openai::responses::ResponsesStreamFn;
use manox_harness::types::{ContentBlock, Model, ThinkingKind};
...
async fn main() {
    ...
    println!(
        "model={} shape=responses kind={:?} level={:?}",
