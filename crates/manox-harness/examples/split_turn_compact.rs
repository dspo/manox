...
use manox_harness::compaction::{CompactionSettings, NothingToCompact};
use manox_harness::session::Session;
use manox_harness::session::jsonl::{JsonlSessionMetadata, JsonlSessionStorage};
use manox_harness::types::{AgentMessage, ContentBlock, StopReason, Usage};
use manox_harness::{AgentContext, AgentEvent, AgentHarness, StreamFn};
...
impl StreamFn for SummaryOnlyStream {
    async fn stream(
        ...
        assert_eq!(
            ...
            manox_harness::compaction::SUMMARIZATION_SYSTEM_PROMPT,
...
async fn main() {
    ...
    let tokens_before =
        manox_harness::compaction::estimate_context_tokens(&harness.agent().state().messages)
            .tokens;
