                            ...
                            .child(
                                gpui::div()
                                    ...
                                    .child(if editor_preview {
                                        // The TextView caches its parsed document per
                                        // element id and early-returns in set_text when
                                        // the source is unchanged, so resizing the pane
                                        // would leave the laid-out tree stale. Derive the
                                        // id from the quantized pane width so a width
                                        // change mounts a fresh state and re-parses at the
                                        // new wrap width.
                                        let preview_id = format!(
                                            "editor-preview-{}",
                                            (f32::from(editor_width) as i32) / 8
                                        );
                                        v_flex()
                                            .h_full()
                                            .p_4()
