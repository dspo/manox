...
//!
//! The rail's agents section renders these under both harnesses; the retired
//! manox harness additionally drills into a per-child panel
//! (`super::subagent_panel`), which the pi harness has no child-thread
//! entities for.
...
pub(crate) struct SubagentInfo {
    pub id: String,
    /// The spawning sub-agent's id for nested trees. Flat under the pi
    /// harness (sub-agents are ephemeral nested sessions, never nested
