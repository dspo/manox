//! The loopback listener: binds `127.0.0.1:<port>`, serves the `/ahp`
//! upgrade route, and hands every accepted socket to the AHP host.
//!
//! Auth: every client must present the per-boot token (`?token=`, `?tkn=`, or
//! a bearer header); browser clients additionally pass the same-origin check
//! (a present WS Origin must be this listener's own loopback origin —
//! DNS-rebinding/CSWSH protection). A missing Origin marks a non-browser
//! client, for which the token alone authenticates.
...
struct AppState {
    token: String,
    port: u16,
    /// The AHP host this listener serves: one process, one host, one
    /// `serverSeq` domain for every client.
...
pub(super) async fn bind_and_serve(port: u16) -> std::io::Result<()> {
...
fn build_router(state: AppState) -> Router {
    // The gate is mounted HERE rather than taken from `AhpRuntime::router`:
    // that helper serves a bare upgrade, and the token + origin checks are
    // this listener's own job. Mounting the raw route would expose the host
    // to any local page.
    Router::new()
