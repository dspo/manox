//! A WebSocket-backed [`RpcConnection`]: clients speak the typed
//! `FromClient`/`FromServer` protocol directly over the socket — no
//! translation layer in between.
//!
//! One connection carries the whole protocol for one client. Inbound WS
//! text/binary frames are parsed as `FromClient` and fed to the AgentServer;
//! `FromServer` messages are serialized back to the socket. The client is
//! expected to send the `Initialize` handshake first (§D.2).
//!
//! `AgentServer::accept` is `&self`/`Sync`, so a connection can be accepted
//! straight from the WS handler without routing through a pump task.
