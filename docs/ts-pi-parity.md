...
| HarnessEvent 面 | ✅（S26，2026-08-04） | 补 `SavePoint{had_pending_mutations}`（session 写入已落地的检查点；在 Rust 实际 flush 点发出，与 TS 挂在 `turn_end` handler 的时机不同——不为对齐而重构 flush，那会动 S8/Phase-4B 立起的持久化粒度不变量）与 `Abort{cleared_steer,cleared_follow_up}`（带回被丢弃的排队消息，用户未发出的输入可恢复；`Agent::clear_queues` 改返回排空内容，新增 `PendingMessageQueue::take_all` 无视 queue mode 全取——abort 丢整个队列而非一轮的量）。`RetryEvent` 补 `operation: RetryOperation{Turn,Compaction,BranchSummary}` 与 `AttemptStart`：三处无关的重试点原先事件同形，observer 无从区分；`AttemptStart` 仅在退避真正走完时发，被取消的退避从 `Start` 直接到 `End`。`subscribe_harness` 返回 `HarnessSubscription`，drop 即退订（比 TS 返回闭包更强：调用方忘了也不泄漏）|
...
| Session move_to/label/name/stats/pagination/custom | ✅（S25，2026-08-04） | `Session::move_to/append_label/set_session_name/custom` + 读侧 `label`/`name`/`find_entries`/`page`/`stats`。`SessionStorage` trait 补齐 TS 读面：`get_label`（同 target 后者胜、blank 即清除）/`get_session_name`（取最新、trim 后空为未命名）/`find_entries(EntryType)`（复用既有 `entry_kind` 判别式，不引第二个）/`get_session_stats`；`get_entries` 改收 `SessionEntryCursor{after_entry_seq,limit}`（越界返回空而非报错，poller 可跨 append 持有游标），**替换**原 `Session::paginate`（newest-first + has-more 的 Rust-only 面，零调用者，同 S2 删 `search()` 之理）。`SessionStats` 语义原地替换为 TS 的 `{messageCount,cachedTokens,uncachedTokens,totalTokens,costTotal}`：聚合 assistant + compaction + branch_summary 的 usage（后两者也是花了钱的模型调用），无 cost 的 entry 其 token 不计但 messageCount 仍加，且**按四类求和而非取 provider 报的 total**（只有部分形状填 total，取它会让不同形状口径分叉）|
...
| 溢出分类 | ✅（S23，2026-08-04） | 25 条锚定正则逐条对齐 TS `OVERFLOW_PATTERNS`（`RegexSet` + `LazyLock` + `case_insensitive`，一趟扫完并去掉每次分类的 `to_lowercase` 分配）；原 20 条裸子串中比 TS 松的已收紧（`exceeds the maximum` → `input token count.*exceeds the maximum` 等），消除「temperature exceeds the maximum allowed value」一类假阳性——假阳性把可重试失败路由进不可重试路径、白花一次压缩并耗尽重试预算，代价远高于假阴性。排除集为**有意更宽**（见 §9）。413 短路保留（status 强于 body 匹配，且空 body 413 无文本可匹配）；terminal/mid_stream 两构造点统一 ProviderError::Overflow |
...
| bashExecution 角色 | ✅（S24，2026-08-04） | `AgentMessage::BashExecution` 一等变体（TS wire 形状 camelCase：command/output/exitCode/cancelled/truncated/fullOutputPath/excludeFromContext），非 eager 投影——TS 就把它作 `role:"bashExecution"` 的 message entry 落盘，投影会让 Rust 读不了含 bash 的 TS session 且丢掉必须跨 reopen 存活的 `excludeFromContext`。`bash_execution_to_text` 与 TS 逐字节一致（四种尾注）；`convert_to_llm` 投影为 user 文本、`exclude_from_context` 时整条丢弃；`estimate_tokens`/`serialize_conversation` 计模型可见的投影而非原始 capture（被withheld 者计 0，避免幻影压缩）。facade `execute_bash`/`abort_bash`/`is_bash_running`（复用 `ExecutionEnv::exec` 的 CancellationToken 进程组树杀 + 新增 `truncate_tail` tail-only 截断 + 溢出 spill 到 session 目录）。**不做流式 `onChunk`**（见 §9）|
...
| AgentSession open | ✅ | `open()` 返回前自动 `restore()` 并复用完整装配；`options.model > restored model > initial model`（显式 model 不被 restore 覆盖）；catalog miss 回退初始模型 + `modelFallbackNotice` |
...
| ResourceLoader | ✅（S5/S10/S13） | AGENTS/CLAUDE ancestor chain（root→cwd）+ agentDir + `.pi/skills|prompts` + settings paths（file-or-directory）+ frontmatter + 冲突 first-wins + 去重 + trust 门控 |
...
| facade getter 面 | ✅（S27，2026-08-04） | 补 `is_streaming`/`is_idle`（同步谓词，`wait_for_idle` 只能等不能问）/`session_id`/`session_name`/`steering_messages`/`follow_up_messages`/`clear_queues`（返回被清内容）/`context_usage`。`context_usage` 移植 TS compaction-trust 规则：压缩边界后无「非 Error/Aborted 且 `calculate_context_tokens > 0`」的 assistant 时返回 `tokens: None`——最新 usage 描述的是压缩前上下文，报出来会把已不存在的历史计进去；边界后有 assistant 应答即恢复可知。复用 `estimate_context_tokens`/`calculate_context_tokens`/`get_branch`/`rposition` 边界扫描，仅 trust 规则是新的。**明确拒绝** `is_compacting`/`is_retrying`（`phase()` 已返回 5 态，两个各塌缩一态的 bool 信息更少还多一份同步义务）与 `get_tool_definition`（TS 唯一非 extension 调用者是 export-html，排除项；`tools()` 已暴露所需）|
...
- **`execute_bash` 无流式输出**（S24）：TS `executeBash(command, onChunk)` 经 `BashOperations.exec({onData})` 边跑边推 `bash_execution_update` 事件。`ExecutionEnv::exec` 无 `on_data`，加它要破 trait + `TokioExecutionEnv` + `BashTool` + 每个 test double，只为喂一个非 TUI 消费者不读的事件。连带后果：取消/超时时 `exec` 丢弃已写出的部分输出，故录到的 `output` 为空而非 partial（注释已就地说明）。`full_output_path` 写 session 目录而非 `tmpdir()`（crate 无 tmpdir 依赖，session 相对路径活得更久）
- **溢出排除集比 TS 宽**（S23）：TS 的 Bedrock 排除锚在 `^Throttling error:`，那是它自己 `formatBedrockError` 产出的散文；本 crate 不含 Bedrock adapter，看到的是原始 AWS body `ThrottlingException: Too many tokens, please wait before trying again.`——不满足该锚点，却匹配 `too many tokens`。故保留未锚定的 `throttl`/`please wait`/`quota`/`insufficient` 并叠加 TS 那条锚定的。照搬 TS 三条会引入 TS 自身注释里警告的那个假阳性；回归测试 `raw_bedrock_throttling_is_not_overflow` 钉住此决策
...
5. ~~coding-agent facade 纵向~~：open 自动 restore、typed credential、fork/事件/shutdown 已闭环（S6/S10）；model catalog 默认表为精确已知模型，未知由 consumer 注入 registry（S12/S13）
6. **pi-ai breadth**：已选三协议之外的 7 个 chat API + image API 未实现（明确排除项，不阻塞 agreed scope）
7. **upstream delta `4488ad55c..origin/main`（已推进至 `c1019d920`，审计于 S7/S10/S16/S17）**：① session storage repository 重构（jsonl-store→jsonl-repo、memory-repo、search.ts，applicable unported delta）；② bounded branch queries + SQLite branch caching（Rust 已移植查询语义，SQLite 缓存未做）；③ harness v2 文档（设计，未实现）；④ coding-agent 修复（connection timeout、availability refresh、model-runtime）；⑤ post-login model catalog refresh 15s 超时与取消信号（env-only scope 下 deferred）；⑥ b9d360a2c 新增仅 Google adapters transient retry（未选 provider breadth）；⑦ c1019d920 `shouldStopAfterTurn`：~~Rust `types.rs` 已有底层形状，`create_loop_config` 仍硬编码 None，未接公共路径~~ **S20（2026-08-04）已对齐**：`create_loop_config` 转发 `loop_hooks.should_stop_after_turn`（不再硬编码 None），签名升至 TS `ShouldStopAfterTurnContext` parity（`message/tool_results/context/new_messages` 四个 `&` 参数，隐式 HRTB 使闭包可 box；修 latent bug——原传 `context.messages.last()`（工具结果），TS 传 turn 的 assistant `message`）；upstream 暴露 commit `0f5286d8a`（2026-08-03）；回归测试覆盖 true 停在首轮且收到 assistant ToolUse 消息、false 续跑第二轮；⑧ c1019d920 tool-result image normalization：Rust 已支持 `ContentBlock::Image` 与三条 provider 图片传输，但无进 session 前的归一化（默认七工具不产图，对自定义 screenshot/MCP 工具是 operational gap）。基线冻结在 `4488ad55c`，以上拆为独立 ledger 项，不夹带已验收切片
...
| 轮次 | Rust 基线 | TS 基线 | 结果 |
|---|---|---|---|
| S29（覆盖率审计轮，2026-08-04） | `ca37c0e` | `4488ad55c` | 按「多做/做错/自由发挥」三类审计全 crate。**覆盖率（按 TS 行数、排除已确认排除项）：`agent/src` 范围内 8721 行——83% 完整移植 + 14% 部分 + 3% 缺口 ≈ 92.5%；`coding-agent/src/core` 范围内 16889 行 ≈ 100%（仅 `auth-guidance.ts` 25 行未覆盖）。** 查出并修 3 处真问题：① `grep` 输出绝对路径（`resolve_path` 把相对参数拼到 session cwd，每条匹配都带完整根；TS `formatPath` 相对搜索根、单文件退化到 basename）——`find.rs` 早已用 `strip_prefix` 做对，只 grep 漏了；修后 remora 又查出单文件时 `strip_prefix` 剥成空串致路径整个消失（探针实测 `":1:fn needle() {}"`），补 basename 回退。② **trust store 读不了 TS 写的文件**：TS 清除决定写 `null`（`Record<string, boolean|null|undefined>`），Rust 按 `HashMap<String,bool>` 反序列化 → 任何含已清除条目的真实 trust 文件硬报错加载失败；改 `Option<bool>` 后 `None` 读作 undecided。③ 凭空发明的默认上限：grep 200→100、find 200→**1000**（后者方向危险：本该 800 结果的搜索静默截到 200 且无提示）。**驳回 remora 一个 P0**（称 TS 的 `null` 会中止祖先上溯、Rust 继续上溯是安全分歧）——直接执行 TS `findNearestTrustEntry` 自身逻辑验证：它只在 `value === true || value === false` 时返回，`null` 落空继续上溯，`{"/a":true,"/a/b":null}` 下 `/a/b/c` 解析为 **Trusted**；按 remora 的「修法」反而会引入分歧。已写测试 `a_cleared_decision_does_not_stop_the_ancestor_walk` 钉住真实语义，防止后人按直觉改错。hashline / 进程内 grep-find / `output_guard` / `max_turns` / `stream_resolver` / `TurnUpdate::appended_messages` 经审计确认为有案增强或承重设计，保留不动。538→540 |
| S28（对抗审查轮，2026-08-04） | `63d9174` | `4488ad55c` | remora 三轮交锋，查出并修 2 个真 bug：① **`message_entry_ids` 索引失配**——三条「无 `MessageEnd` 却增长 transcript」的 append 路径都没记 entry id，索引比 transcript 每次短 1；compaction 的 `first_kept_entry_id` 读越界得 `None`，restore 时走不到树上任何 entry，反丢掉压缩本想保留的消息。修法取 `session.append_message` 返回的真 id（而非 `None` 填充），先探针复现（transcript=5 vs ids=4）再修。② **`emit_harness` 持锁回调致死锁**——`std::sync::Mutex` 非可重入，listener 在自己回调里 drop 订阅会取同一把锁；用独立最小复现程序确认线程挂死后，改为先克隆 listener 再回调（对齐 `agent.rs` 既有分发写法）。驳回 remora 一个 P1（`get_session_stats` 未计 `ToolResult.usage`）：TS `jsonl-storage.ts` 显式限定 `role === "assistant"`，加该臂反破坏 parity，remora 复核后撤回。终轮 5 个攻击向量（多轮 id 顺序 / 部分 flush 一致性 / 克隆回调语义 / `prepare_for_wire` 覆盖 / 锁序）全部不成立；per-position 对齐经本地探针独立验证后写成断言。534→536 |
| S27（2026-08-04） | `feat/pi-harness-event-surface` | `4488ad55c` | facade getter 面（见 §8）。顺带统一队列清空 API：`Agent::clear_all_queues` 与新加的 `clear_queues` 语义重叠，删前者留后者（返回被清消息），shutdown 路径改发 `HarnessEvent::Abort`——原先 shutdown 清队列不发事件，与 abort 不一致。`Scripted` 测试 stream 原报零 usage，导致任何 context-usage 断言都落在「不可信」分支；改为报非零 usage（真实 provider 行为），529→534 全绿证明未扰动既有断言。三个「拒绝」项写进 §8 而非默默不做 |
| S26（2026-08-04） | `d2fca08` | `4488ad55c` | HarnessEvent 面补 `SavePoint`/`Abort`，RetryEvent 补 `operation` 判别式与 `AttemptStart`，`subscribe_harness` 返回 drop 式退订句柄。`RetryOperation` 是本项主要收益：Rust 单一 RetryEvent 从 turn/compaction/branch-summary 三处发出，observer 原先无法区分一个失败的回合与一个失败的摘要调用。顺带清掉 `is_transient_error` 上重复粘贴的 doc comment。`flush_pending_mutations` 拆出 `_inner`，save point 无论队列是否干净排完都发出（部分 flush 也推进了持久状态），并经 `flushed_any_mutation` 把 mid-turn 边界已排空的工作计入本次 save point。526→529 |
| S25（2026-08-04） | `feat/pi-bash-execution-role` | `4488ad55c` | `SessionStorage` 读面对齐 TS：新增 `find_entries`/`get_label`/`get_session_name`/`get_session_stats`，`get_entries` 改收 `SessionEntryCursor`。`SessionStats` 从 `{entries,messages,branches,last_activity}`（Rust 自创的结构计数）原地换为 TS 的成本聚合五元组——两个调用点（facade `stats()` + `coding_agent_smoke` example），无外部数据受损，按 CLAUDE.md 激进纪律直接替换不并存。刻意不复用 `calculate_context_tokens`（它在 `total_tokens` 非零时优先取 total，而 TS 无条件四类求和；复用会在每条 Responses 消息上分叉），回归测试 `session_stats_sum_the_classes_not_the_reported_total` 钉住。TS 的「任一字段非 number 则跳过」在 Rust 唯一可表达形式是 `cost.is_none()`（四个 token 字段非 Option）。删 `Session::paginate`（零调用者的 Rust-only 面）。`entry_kind` 提升为 pub 供两处共用。518→526 |
| S24（2026-08-04） | `2a9c2fe` | `4488ad55c` | `bashExecution` 一等消息角色（8 个 exhaustive match 点，无 catch-all）+ facade `execute_bash`/`abort_bash`/`is_bash_running` + `truncate_tail`。顺带修两个 latent bug：① `append_message` 只写 session 不写 agent transcript，被 append 的消息在本次会话内对模型不可见（TS `recordBashResult` 两处都写）——idle 路径改 session-first 再 `append_to_transcript`；② `HarnessHandle` 无 `append_message`，mid-run append 无公共入口，原 `test_append_message_idle_immediate_running_queued` 的 mid-run 半边是空壳（listener 里 `let _ = handle`，末尾断言重复检查 idle 那条）——补 handle 方法，并让 `prepare_next_turn` 的 mid-run flush 经 `TurnUpdate.appended_messages` 喂回 loop context、经 `flushed_messages` 喂回 transcript，测试改为真实断言「bash 消息落在本轮最后一个 tool result 之后」。`TurnUpdate` 去掉未被使用的 `PartialEq/Eq` derive（`AgentMessage` 无 `Eq`）。500→518 |
| S23（2026-08-04） | `d6c93e9` | `4488ad55c` | overflow 分类器由 20 条裸小写子串换为 TS 的 25 条锚定正则（`RegexSet`+`LazyLock`+`case_insensitive`，删 `to_lowercase`）。收紧的四条原比 TS 松：`exceeds the maximum`/`maximum context length`/`reduce the length`/`prompt too long`，其中第一条会命中「invalid temperature: exceeds the maximum allowed value」；另删无 TS 对应的 `payload too large`/`input length should be`/`context window exceeded`/`reduce your prompt`/`input too long`/`request too large`。方向不对称是取舍依据：假阳性把可重试失败路由进 `ProviderError::Overflow`，白花压缩且弃掉重试预算；假阴性只是不自动压缩。排除集有意保留更宽（§9，Rust 无 Bedrock 规范化层）。新增 5 测试（keyword-only 不再假阳性、15 种厂商数字形态、Cerebras 空 body 仅锚定命中、raw Bedrock throttling、Service unavailable 前缀）；既有 19 条 overflow/harness 溢出测试原样通过，证明改动限于 `classify`。495→500 |
...
| S20（复核轮，2026-08-04） | `1d0241e` | `4488ad55c` | `shouldStopAfterTurn` 接公共路径（parity doc ⑦，upstream 暴露 commit `0f5286d8a`）：`create_loop_config` 转发 `loop_hooks.should_stop_after_turn`（原硬编码 None，TS seam 不可达）；签名升至 TS `ShouldStopAfterTurnContext` parity——`(message, tool_results, context, new_messages)` 四个 `&` 参数（隐式 HRTB 使消费者可 box 闭包；生命周期参数化 context struct 会让闭包→`dyn` HRTB 推断失败，故取 positional `&` 对齐 `before_tool_call` 习语）；修 latent bug——原传 `context.messages.last()`（工具执行后是工具结果），TS 传 turn 的 assistant `message`，TurnEnd 前克隆 `message` 跨 emit 保留；回归：hook true 停在首轮且收到 assistant ToolUse 消息（非工具结果）、AgentEnd 发出；hook false 续跑第二轮 |
| S19（复核轮，2026-08-04） | `43100a8` | `4488ad55c` | coding_agent_smoke example 隔离 temp agent_dir（原写真实 `~/.pi/agent/settings.json`，默认 example gate 失败）；build.yml 接 `check_examples.sh` 进 CI（offline example gate 强制）；`patch_global_settings` 并发安全：per-agent-dir `tokio::sync::Mutex` 横跨 read-modify-write-rename + 唯一 tmp 名（pid+counter），消除两 session 共享 agent_dir 时的丢更新/temp 碰撞（无跨进程锁，单进程交付）；`set_model` 加 TS checkAuth preflight（`(runtime.resolver())(model)` 在任何 mutation 前 surface `MissingCredential`）+ settings 改 LAST（switch/entry 之后），rejected switch 与 entry 失败均不污染 settings；合并 defaultProvider/defaultModel/defaultThinkingLevel 为一次写。回归测试：credential preflight rejected 后 model/session/settings 全不变；20 轮两 session 并发 patch 不同字段无丢更新 |
| S18（复核轮，2026-08-03） | `477a8e7` | `4488ad55c` | 全局 settings 改字段级 patch（`patch_global_settings`：原始 JSON 读改写 + temp+rename 原子落盘，未建模字段 theme/packages/extensions/transport/images/enabledModels 及未来键全保留，对齐 TS SettingsManager merge-only-modified）；显式 `off` 持久偏好保留 wire `Some("off")`（runtime tier 仍 None==off），跨非 thinking 模型往返 + fork 后恢复 off 不回退 medium；`append_thinking_level_change` 失败用 `?` 传播（set_model/set_thinking_level 均改），内存 tier 仅在 entry 写入成功后更新，消除 runtime↔持久态分叉；`reopen_keeps_model_on_its_original_api_via_catalog` 注入 temp agent_dir（原写真实 `~/.pi/agent/settings.json`），审计确认所有调用持久 setter 的测试均用 temp agent_dir。remora 对抗两轮：自审补 settings 解析错误传播（不再静默 wipe 成 `{}`）+ 非 object root 不再 `__legacy_root__` 包裹；remora 轮 1 补 `create_dir_all(agent_dir)`（首次用户无目录时 set_model 不再 ENOENT）+ defaultThinkingLevel best-effort 写失败改 `tracing::warn`（switch/entry 已落地，不可假失败）；settings-first 顺序保留（TS setModel 契约，session JSONL reopen 权威），无跨进程锁（单进程交付） |
| S17（复核轮，2026-08-01） | `f9830c9` | `4488ad55c` | `set_thinking_level(None)` 恢复 `off` 语义（契约不随 clamp 改变）；thinking 变更仅在 effective level 变化时持久化（TS setThinkingLevel，high→high 切换零多余 entry）；preference 经 settings 默认持续更新并贯穿 fork（TS setDefaultThinkingLevel/setDefaultModelAndProvider 落全局 settings 文件）；set_model 先落 settings 再切模型（无半次切换） |
| S16（复核轮，2026-08-01） | `efa8ae0` | `4488ad55c` | facade set_model 重新 clamp thinking 并持久化（TS `_getThinkingLevelForModelSwitch`：thinking→non-thinking 变 off、non-thinking→thinking 恢复 settings 默认）；set_thinking_level 亦 clamp；`ModelCatalog.supported_thinking_levels` 可表达 thinkingLevelMap（自定义 catalog 限制 xhigh/max）；default_model 未知 ANTHROPIC_MODEL 整退回 Sonnet；custom_prompt 布尔贯穿 fork；serde_json/preserve_order 消除 workspace feature-unification 的 compaction 序列化差异（workspace 全绿） |
| S15（复核轮，2026-08-01） | `4a73759` | `4488ad55c` | default_model 经 catalog 解析（Sonnet thinking=Enabled，默认路径不再 clamp medium→off）；`clampThinkingLevel` 按 model 支持级别集合（TS 顺序上/下搜索；Sonnet 4.6 排除 xhigh、Opus 4.8 adaptive）；persisted `off` 在 reopen 保持 off（不再回退 medium）；custom prompt rebuild 保持替换语义（builder 捕获 custom 标志）；显式 model B reopen 真实 prompt 断言 B 服务 |
| S14（复核轮，2026-08-01） | `2f2f29a` | `4488ad55c` | open 显式 model 优先（不装 model_resolver，TS `options.model > restored`，回归测试）；thinking clamp 统一为 `clampThinkingLevel(final model, desired)`（catalog 按 TS 基线标注 Sonnet 4.6/Opus 4.8/GPT-5 为 thinking-capable 保留 medium；session 持久化 high + fallback 非 thinking model → off；首轮==reopen）；prompt 补 bash-only 动态 guideline、隐藏无 snippet 工具、customPrompt 替换语义；upstream 前进至 `ebf33c0c2`（仅 UI 类型，无 non-UI 漂移） |
| S13（复核轮，2026-08-01） | `957533d` | `4488ad55c` | thinking 最终 clamped 值同用于初始 entry 与首轮内存态（首轮==reopen）；settings 默认 high + 非 thinking model clamp 为 off；四工具初始 active subset 驱动初始 prompt 且为 reopen 无 entry 时的 restore 默认（七工具 registry 保留）；model resolution 统一链（explicit → exact restored+catalog → settings/catalog initial → fallback notice，删除 api_for_provider 手工猜测）；facade shutdown 清 pending next-turn 并拒绝再入队；默认 prompt 移植 TS buildSystemPrompt 核心（identity/guidelines/tool snippets/<project_context>）；补 9 个纵向回归（facade next-turn 顺序、七+四工具、默认四 reopen、thinking 首轮/reopen、catalog fallback、fork builder、shutdown queue 等） |
...
| S11（复核轮，2026-08-01） | `0443948` | `4488ad55c` | trust 先于 settings（untrusted 项目 settings 视为空配置，含回归）；default thinking 双状态（agent + turn_runtime）且 reopen 无 thinking entry 时回落 settings default；默认工具集改 TS 四件（read/bash/edit/write）；system prompt 随 active tools/resources 重建（builder 闭包，关闭 read 后隐藏 skills）+ skill XML 转义；reopen 模型 catalog 未命中回退初始模型 + `modelFallbackNotice`（不再猜协议）；next_turn 顺序改 user 在前（TS coding-agent）；`prompt()` 默认展开 `/skill:` 与 `/template`；tree hook summary 仅在 summarize 时接受 + fromHook 贯穿持久化与事件；skill/template collision 保留先加载（TS winner=first）；env 测试共享锁消除并行污染 |
