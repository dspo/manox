...
| S17（复核轮，2026-08-01） | `（待提交）` | `4488ad55c` | `set_thinking_level(None)` 恢复 `off` 语义（契约不随 clamp 改变）；thinking 变更仅在 effective level 变化时持久化（TS setThinkingLevel，high→high 切换零多余 entry）；preference 经 settings 默认持续更新并贯穿 fork（TS setDefaultThinkingLevel/setDefaultModelAndProvider 落全局 settings 文件）；set_model 先落 settings 再切模型（无半次切换） |
