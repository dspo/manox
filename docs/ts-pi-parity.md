...
| S18（复核轮，2026-08-03） | `08df7fb` | `4488ad55c` | 全局 settings 改字段级 patch（`patch_global_settings`：原始 JSON 读改写 + temp+rename 原子落盘，未建模字段 theme/packages/extensions/transport/images/enabledModels 及未来键全保留，对齐 TS SettingsManager lock+merge）；显式 `off` 持久偏好保留 wire `Some("off")`（runtime tier 仍 None==off），跨非 thinking 模型往返 + fork 后恢复 off 不回退 medium；`append_thinking_level_change` 失败用 `?` 传播（set_model/set_thinking_level 均改），内存 tier 仅在 entry 写入成功后更新，消除 runtime↔持久态分叉；`reopen_keeps_model_on_its_original_api_via_catalog` 注入 temp agent_dir（原写真实 `~/.pi/agent/settings.json`），审计确认所有调用持久 setter 的测试均用 temp agent_dir |
