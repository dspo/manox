...
| S9（复核轮，2026-08-01） | `d86a6fe` | `4488ad55c` | build/open 共用 assemble（open 按 session model 经 catalog 解析，不再默认 Anthropic 阻塞；AgentSession.cwd 跟 session；e2e build→close→换 process cwd→open→fork 不漂移）；ModelRuntime.resolve_model 为唯一 catalog；branch query 重写为 upstream 语义（顺序感知 traversal/stop 边界/custom-type 过滤 Custom/limit 正数校验/cycle+断链 invalid_session/start null→[]）+ 移植 branch-query.test.ts；唯一 system-prompt builder（context files + skill index）；settings.skills/prompts→ResourceLoader、shellCommandPrefix→BashTool、showCacheMissNotices、branchSummary.reserveTokens、2048 maxTokens；before-payload 链式传递 + after-response headers；branch-summary retry 仅瞬态错误 + RetryEvent 生命周期 + 溢出安全退避 |
