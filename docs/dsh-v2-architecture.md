...
| L2 编排层 | `workspace.rs`（生产 5.3k）+ `workspace/attach.rs`（807，簇1）+ `workspace/plan_review.rs`（288，簇2）+ `workspace/composer.rs`（946，簇3）+ `workspace/external.rs`（1098，簇4）+ `workspace/right_pane.rs`（814，簇5）+ `workspace/chips.rs`（1108，簇6）+ `multiplexer.rs` | workspace=UI 编排（attach 生命周期/面板草稿/park/棘轮冻结面）；multiplexer=**唯一 wire 面**（gateway client：calls/notes/fold） | `no_new_files_touch_wire_or_store`（豁免表显式有限：workspace/multiplexer/slash_command/source_gates/client_store_handle/workspace\tests.rs）+ 棘轮预算表（SENDS 24/STORE_WRITE 7/STORE_GLOBAL 10/FACADE_WRITE 2） |
