...
| 层 | 文件面 | 职责 | 门禁 |
| --- | --- | --- | --- |
| L1 视图层 | `views/*.rs`（17 文件） | 纯渲染 + 本地交互态；不触协议发送面、不持内核对象句柄 | `views_never_touch_wire_or_kernel_handles`（sends 恒 0；ThreadHandle 冻结预算 3=context_rail 的 doc/field/ctor，U7b 迁移项，只减） |
