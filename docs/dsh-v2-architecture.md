...
> **仓库边界(用户裁决,本 PR 收敛)**:本仓仅维护纯 Rust 面(GPUI 桌面+网关+内核+napi 绑定)。web 前端(apps/web/webui)与 VS Code 扩展(apps/vscode)已整体删除,最终形态存档于 tag `archive/frontends-final`;`crates/manox-webui` 删除,其通用 WS 传输面存续为 `manox-session-core::ws`(`cx web` 无头启动,端点写 `<config>/gateway-ws.json`);napi 绑定迁 `crates/manox-napi`(休眠保留,供未来宿主重实现)。文中一切 webui/vscode 行为描述为 as-built 历史记录;凡以其存活为前提的跨端 gate(C4b 大宗删除、compat 三件、badge 迁移、§J.1 的 npm 腿)均按 §K.7.2 标注解除或退役。
...
**as-built（arch 审计修订）**：拆除实际删 37 留 11（保留集＝owner 控制 `ready/sessionCreated/sessionDisposed`、过渡列表通道 `threadsUpdated/models/commands`、服务端 `error`、ModelChat 侧流 ×4——以 surface.rs 的 `SERVER_NOTES` 宏生成清单为准）。§J.6 的「零残留」声明不实：compat `ClientNote::{CreateSession,Submit,Steer}`：Submit/Steer 生产发送方已清零;CreateSession 仍有桌面 `open_or_create(reopen=false)` 生产腿（P0-6 普查更正——原「桌面 landing 已走 v2」说不实）,vscode（已删除,仓库边界裁决）曾是另一发送方；错误桩 `ClientCall::{GetUsage,GetCurrentModel,ThreadInfo}` 已于 C4b-narrow 删除（零发送方，grep 证据）。C4 关闭双协议窗口时经 surface 宏清单一次删除（穷举 tag match 使表/样本/类型同步收敛，编译期门禁）。GW1 已落地：8 个 HostEvent 变体全部有生产发射点（与 v1 note 同受众双发）。客户端迁移：webui store 折叠 Host 镜像✓；**桌面 U2 已落地**——multiplexer `apply_host` 全量镜像（Ready{epoch} 校验 PROTOCOL_EPOCH+触发 ListThreads/ListModels/ListCommands 首拉、Models/Commands/ThreadsUpdated 替换、SessionStatus 按 §D.5 客户端单调规则逐条对齐 webui、SessionCreated/Disposed/Error 权威已翻转（C4a-1：host SessionCreated 臂承担 leaf 创建+has_follow 守卫的 follow 首开，leaf Host 臂将三类控制帧规范化入 note 路径，note 臂仅对账；HostEvent::Error 携 session_id 作用域）），sidebar/列表面零内核读✓；C4b 大宗删臂原 gated 于 vscode host 迁移——该项目已整体删除(仓库边界裁决),gate 解除,归仓内 follow-up。
...
4. 声明面覆盖（L12）：journal 条目/投影 key/host 事件/协议帧四张表，emit 点 100%（coverage 测试：脚本化会话驱动后断言每个声明面出现在 FromServer 流；扩展 `dual_path_transport_consistency`）。**口径（round 3 OPEN-1 入文）**：本门与 J1b 证明的是「声明面在真实网关流上可达」——脚本化会话驱动真实 dispatch、J1b 走真 CreateSession 组合；不是「内核生产路径逐变体真发」——后者由 §C.2 词汇表与 `wire_projection_covers_every_kernel_variant_totally` 的 38/38 全投影锁保证（内核变体 ⇒ wire 臂 ⇒ JOURNAL_ENTRIES 行 ⇒ 样本，四处编译期+测试期联动）。
...
> **Wave 2 收官声明(J2)**:范围内清偿项全部落地并逐项带回归+门禁+变异红——U6a(桥退休)/U6b①-⑥(gateway 迁移全图)/U7b(rail 内核句柄归零)/U9a+U9b(god-file 拆解 −76%,九文件棘轮总量守恒 24/7/10/2)/plan_review 日志化/goal 权威三段迁移/K5-edge/GW6-neighbor/K2-title/跨域 #5。余债=C4 门控批(**原硬前置「vscode extension-host 迁移」已随项目整体删除失效(仓库边界裁决),gate 解除,以下各项归仓内执行**:U3b 新 call、C4b 大宗删除、C2 RpcOutcome 类型化、C5 SetModel ModelRef、unread 列+badge)+ 观察项(Ready 快照 pin 覆盖,自愈)。

| 债务 | 现状 | 归属 |
| --- | --- | --- |
| U3b:用户动作写无网关调用(archive/tag/remove_project/register_project 直写 store) | 需协议新 call(与 C4 面一起设计) | C4 |
| U6(**桥已退休✓ U6a**:服务端 store watcher 广播列表刷新——§D.5「元数据变更时推送」由虚转实;桌面 store 订阅/_store_pump 字段/on_thread_store_changed 删除,u2(b) 改显式驱动 wire 腿(desktop 套件 global-singleton server 的 watcher 订阅首个构建者的 store,生产次序无此虞:store init 先于首个 workspace);STORE_GLOBAL 棘轮**持平 14**——thread_store 绑定为 attach 面读点存活)。**U6b 实施地图(①-⑥ 全落地✓;⑥ 余面=store-API 侧设计内存活)**(普查已定界):workspace `self.thread` 29 处(with_mut×6/read×4/clone×3+散布读)——①browser-suite 开关 **✓ 已落地(U6b①,形状修正:setter NOTE 族而非 call——与 SetPlanMode/SetModel 同族)**:ClientNote::SetBrowserSuite{session_id, suite:封闭 wire 名, enable}+engine BrowserSuite::wire/from_wire+服务端臂(landing 镜像 park/活引擎经 facade→engine cmd;未知 suite 名回 error note 非 panic)+桌面 toggle 迁移(landing 无线可走时 fallback facade park=设计内着陆路径,ensure_engine 重放;棘轮 SENDS 21→23 带理由注记、FACADE_WRITE 持平 5——toggle 主路径已 wire 化,残余=dismiss+2 构造点+2 landing fallback);回归 set_browser_suite_note_lands_on_the_facade+变异红;②open_thread/attach_created_session **✓ 已落地(U6b②)**:attach 句柄恒为 Thread::landing_with_id 分离渲染镜像,restore 走 wire reopen 流(OpenSession+follow Snapshot+§D.5 镜像);load-miss 守卫随退休(行=wire item,id 构造上 server-known);workspace thread_store 句柄整体退休(字段/U6a 绑定/init——最后消费方即两 attach 读);live_thread/register_live_thread API 退休(全仓零消费方),load_thread 存活(网关自身 open 路径=合法消费方,live_threads 缓存为其内部复用);棘轮 STORE_WRITE 11→9/STORE_GLOBAL 14→12/FACADE_WRITE 5→4;③构造点写 **✓ 已落地(U6b③)**:两 create 流全走 v2 CreateSession intent——archive_current_thread_inheriting(/new 继承流,原 compat-note create 只携 cwd,park 的 model/project/effort/permission 从未达服务端=#765 round-2 继承缺陷本形)+ respond_plan_review ExecuteFresh 流(原 Thread::new_* 本地内核构造+facade seed_plan_execution=最后的进程内执行路径,ensure_engine 会 spawn 第二引擎与网关竞同一文件;现 seed turn 骑 PlanSeedExecution note——服务端 plan_seed 渲染同款 render_plan_mode_approved+插入+跑 turn,FIFO 于 attach 的 OpenSession 之后;archive 先于 refetch 的次序保持,移入 intent 回调);plan_seed 气泡 author 由 self_author(Agent) 对齐 MessageAuthor::Harness(变体 doc 自证此用途;桌面本地语义原本即 Harness,webui 同步对齐);棘轮 FACADE_WRITE 4→2(残余=2 browser-suite landing fallback=设计内着陆 park);回归 plan_seed_note_runs_the_seed_turn_server_side+变异红;④dismiss **✓ 已落地(U6b②/④,隐式消解)**:pending review 期间落地的 free-form Submit=消解(用户在讨论/修订而非接受)——持久半边归网关 submit 臂(清 session facade→engine 日志 `resolved` plan_review 边+sidecar;清 store 徽章旗;turn 自身 §D.5 delta 携带清零旗至全客户端),桌面保留 UI 半边(卡片消解/折叠记录/remeasure)并删 facade 写(镜像无 engine,状态属会话;消解 UI note 早已走 wire AppendUiNote);回归 submit_while_review_pending_implicitly_dismisses+变异红;⑤background_threads parking **✓ 已落地(U6b⑤,park 生命周期反转)**:BackgroundThread.entity(强持内核 ThreadHandle)退休为 id:String——park={id, leaf, session_id, 订阅};turn 本体在服务端跑、切换自存活,park 唯一余职=保持会话 ATTACHED 使 reclaim 免 reopen 原地复接;park 判定改读前台 **leaf 的 wire running 镜像**(服务端泵 §D.5 delta 喂)——同时修复②引入的回归(facade is_running 在分离镜像上是死旗恒 false,running turn 切换曾静默走 idle-detach:安全但丢免-reopen reclaim 且中途 detach);park seeds 退休(mark_running/mark_background_work 双写——U3b 单写者);reclaim 全路径随 id(attach reclaim/retain、open_thread reclaim=重建 landing 镜像+attach 内 reclaim 分支原地恢复、两终态 retain、diagnostic_emit_event find);棘轮 STORE_WRITE 9→7/STORE_GLOBAL 12→10;**SENDS 23→24=更正**(U6b③ 的 PlanSeedExecution send 按设计增面但该 commit 漏更预算——其记录的 suite-green 对棘轮腿失实,由 gate 自身 assert 在本批抓获);回归 park_rides_the_leaf_running_mirror_and_reclaim_skips_reopen(leaf running 升起→切换→park,切回→原地 reclaim 同 leaf entity_id)+变异红;⑥直读退休 **桌面侧已完成(U6b②)**:attach 直读清零+thread_store 句柄退休+live_thread/register API 退休(load_thread 为网关消费方存活);③⑤ 均已落地(见上);⑥ 余面=store-API 侧收尾(load_thread 为网关 open 路径读取,设计内存活) | U6b 分步实施 | Wave 2(多轮) |
| ~~vscode:徽章迁移(GW5 后列表 unread 恒 false;focusThread 死帧行已随 C4b-narrow 删除)~~ **resolved-by-deletion(仓库边界裁决:vscode/webui 项目整体删除,终态见 tag archive/frontends-final)** | — | 已关闭 |
| C4b 大宗删除 gate:vscode extension host 消费 note 形状事件(sessionCreated×3/threadsUpdated×2/sessionDisposed×2/ready×2/approve×2/planVerdict/askUserQuestion/models/commands——awaitSession 谓词+审批流);parseFromServer 不归一 host 帧 → 删臂=vscode 会话创建挂起(硬破坏,非 badge 类退化)——**消费方已随项目整体删除(仓库边界裁决),gate 解除** | 原硬前置「vscode host 迁移」失效(仓库边界裁决:该项目已删除);C4b 大宗+compat 三件删除归仓内直接执行,历史消费形状考古见 tag archive/frontends-final | C4 follow-up(仓内) |
| C2:无码错误归码 ✓(错误码纪律+结构门禁 every_production_rpc_error_carries_a_stable_code,aed45a57);RpcOutcome 类型化 → **归 C4b 大宗批**(§D.7 as-built 自注:「波及全客户端 Response 消费面,与 C4 wire 工作同批」;原 vscode host 迁移 gate 已解除(仓库边界裁决)) | C4 | C4 |
| U7b:Q 面 visibility 门控(rail 可见性状态归属) **✓ 已落地**:context_rail 的内核 thread-handle 字段退休(γ-2a 双读迁移的 fallback 遗骸——per-model usage/project/cwd/title 全部读 leaf store,`expect("foreground store present")` 证明无 fallback 残留;字段/ctor 参数/attach rebind 写点/构造调用全删)——views 层内核句柄面**归零冻结**(gate assert 由 ≤3 收紧为 =0);rail 可见性状态归属=leaf store(Q 面唯一读源) | ✓ 完成 | U9 |
| U9:god-file 拆分 + 三层归属表 + 依赖门禁 **U9a ✓ 已落地**:测试面提取为子模块(workspace.rs 12364→10248,tests.rs 2108;agent_server.rs 10117→**3197**——账本 6.7k 数字已陈旧,follow/journal_query/projections/translate 早已独立,3.2k 生产行不再 god;`super` 语义不变,子文件仅在父 `#[cfg(test)] mod tests;` 声明下编译)+ views 依赖门禁落地(K.7-6 终局 grep:views/*.rs 零协议发送针;内核句柄面冻结 3=context_rail ThreadHandle doc/field/ctor→U7b rail 可见性迁移项,只减不增)+ no_new_files 豁免表增 workspace/tests.rs(整文件测试代码,无内层 marker 可截);三层归属表见下 as-built;**U9b 簇1 ✓ 已落地**:workspace/attach.rs(807 行)——attach/park/reclaim 生命周期+create-intent 发射+archive 手势 9 函数(attach_thread/open_thread/attach_created_session/start_new_thread/subscribe_background_thread/archive_current_thread/archive_active_thread_if_idle/archive_current_thread_inheriting/background_current_thread);方法升 pub(super)(子模块私有项父不可见——pub(super) 保父模块与其 tests 子模块可调用),`use super::*` 惯用法(U9a 提取已证);workspace.rs 10248→9453;棘轮随面走:workspace SENDS 24→22/STORE_WRITE 7→5/STORE_GLOBAL 10→8(FACADE_WRITE 持平 2=toggle fallback 留守),attach.rs 冻结于搬入计数 2/2/2/0——**总量精确守恒(纯搬迁非增长)**;no_new_files 豁免增 attach.rs(预算行已列表);**簇2 ✓ 已落地**:workspace/plan_review.rs(288 行)——plan-mode 镜像读/plan-mode 开关/verdict reply+cancel 腿/respond_plan_review 四选编排(refine/execute/execute-fresh=U6b③ intent+PlanSeedExecution 序列/compact)5 函数;通用 wire 助手(send_note/send_submit_v2)留守 workspace(服务全面非仅 verdict 面);workspace.rs 9453→9178;棘轮随面走:workspace SENDS 22→19/STORE_WRITE 5→4/STORE_GLOBAL 8→7(FACADE_WRITE 持平 2),plan_review.rs 冻结 3/1/1/0——总量再守恒(24=19+3+2/7=4+1+2/10=7+1+2/2=2+0+0);**簇3 ✓ 已落地**:workspace/composer.rs(946 行)——composer turn 面 27 函数三段(submit_input 及 turn 构造器/steer+queued follow-up 生命周期(parked stash flush/pending 气泡/stranded-steer 愈合/drop/undo)/recall walk);pub(crate)×3 保持+priv×24 升 pub(super);通用 wire 助手(send_note/send_submit_v2)留守父模块;workspace.rs 9178→8245;棘轮随面走:workspace SENDS 19→16,composer.rs 冻结 3/0/0/0——总量三度精确守恒(24=16+2+3+3/7=4+2+1+0/10=7+2+1+0/2=2+0+0+0);**簇4 ✓ 已落地**:workspace/external.rs(1098 行)——external-view 会话族 22 函数连续区(嵌入 terminal tabs/CLI+ChatGPT+VSCode 外部会话 spawn/resume/watch/attach/place/mount/title/close/remove/browser tabs 开+标题 ticker+可见性+关);pub×10+pub(crate)×1 保持、裸私有×11 升 pub(super);**针计全零(0/0/0/0)——该族从不触 wire/store,纯 UI 面**,四族零预算行=冻结证明;workspace.rs 8245→7159;总量四度守恒(24/7/10/2 不变——本簇零针搬迁);**簇5 ✓ 已落地**:workspace/right_pane.rs(814 行)——右观察面板族 31 函数(terminal/editor/browser/launcher/subagent tabs、面板持久化+每线程 stash/restore、launcher cascade、editor preview/submit 腿、focus_terminal);pub×2+pub(crate)×1 保持、裸私有×28 升 pub(super);针计 0/0/**3**/0(3 个 store global=右面板 threads.db 持久化读写,随面板搬迁并冻结);workspace.rs 7159→6357;棘轮随面走:workspace STORE_GLOBAL 7→4,右面板行 0/0/3/0——总量五度精确守恒(24=16+2+3+3+0+0/7=4+2+1+0+0/10=4+2+1+0+0+3/2=2+0+0+0+0);**簇6 ✓ 已落地**:workspace/chips.rs(1108 行)——chip+交互卡三族合批 28 函数(ask/auth 卡面:resurface/快照/选项 toggle+prev/next/resolve-with-response/auth resolve 腿;model-selector pi 面:wire tag+color/canonical identity+display 解析器×4/selector render+popup builder;goal chip:popover+edit/budget/rounds/replace/new begins+chip render);pub×7+pub(crate)×13 保持、裸私有×8 升 pub(super);针计 **5**/0/0/0(5 send=ask/auth resolve 腿的 wire reply);workspace.rs 6357→5263;棘轮随面走:workspace SENDS 16→11,chips 行冻结 5/0/0/0——总量六度精确守恒(24=11+2+3+3+0+0+5/7/10/2 七文件和不变);**簇7 ✓ 已落地**:workspace/render.rs(1066 行)——chrome 渲染面连续块(impl Render for Workspace + render_manox 全区布局/sidebar/conversation/right-pane/overlays + shell_root 共享布局容器 + render_terminal_column);**针计全零(0/0/0/0)——纯元素构造器,零 wire/store 触达**,四族零预算行冻结;6 个自由辅助函数(thread_cwd/parse_pending_ask/goal_popover_row/strip_recommended_suffix/mode_chip_visual/build_permission_content)回迁父模块(多模块消费,属父命名空间——census 只扫 4 空格缩进方法漏了列 0 自由函数,编译错误点名后归位);workspace.rs 5263→4209;总量七度精确守恒(24/7/10/2 八文件和不变);**簇8 ✓ 已落地**:workspace/composer_render.rs(1317 行)——composer+overlay render 助手群 22 函数(render_composer 各面/browser-suite toggles(携 2 landing fallback)/pick_files/register_project_in_store/blank_project 族/pending_auth_overlay);裸私有×22 全升 pub(super);**workspace.rs FACADE_WRITE 归零冻结**(toggle fallback 随迁);棘轮:workspace 11→6/4→3/4→3/2→0,composer_render 冻结 5/1/1/2——总量八度精确守恒(24/7/10/2 九文件和不变);workspace.rs 4205→2902。**U9b 收官决断**:types 类型园与 Workspace struct 同居=idiomatic Rust 保留(god-file 已拆:12364→2902=−76%,八子模块+tests;余 64 函数=struct+类型园+核心编排=合理尺寸,对照 multiplexer 1747) | U9b ✓ 完成 | Wave 2 |
...
| §二.1 背压出站面（round 3）：WS 载体 `send_blocking` 无出站上界/超时/心跳——对端停读不关连接时 follow 任务永久驻车在发送上，`Lagged→Resync` 语义被无限推迟（休眠面）；内核 feed 容量已命名 `JOURNAL_FEED_CAPACITY` 且与 `ENTRY_BACKPRESSURE_CAPACITY` 等值锁（b6e9989c），但 feed 4096 > 载体 1024 ⇒ 先饱和的是载体。修向 = 出站写上界 + 明确终态 + 心跳；P1-5 Lagged 遥测归此批 | 认可债务 | C4/wire 批 |
| §二.2 分页死承诺（round 3）：`max_messages` 两腿恒 `None`（multiplexer 补页/开流；napi 样本同形）⇒ follow 尾截断不可达、`window_has_more` 恒 false、补页=全链+二次全等 Replace（K8① 已记冷读全链为有意，但 wire 参数承诺与实现不符——实装或删参）；`Some(0)` 无 ≥1 clamp 答「空页+has_more:false」假终止；桌面 row cache 只增不减 | 认可债务 | C4/wire 批 |
| §二.4 网关三窄竞态 **已修（W6b，5c0733ee）**：remove_client 单锁完成代际检查+摘除；track_stream 覆盖旧值必 end(Closed)（不再孤儿）；create 改 `insert_session_if_absent` 单锁复查插入（竞态输家退役自身泵、采纳赢家；死码 `insert_session` 退役）；§D.2 幂等注已翻正。四条回归带变异红 | ✓ 修复索引 | — |
| §二.9 TS 孤儿面 **已按用户裁决整体删除（W6，b6e9989c）**：`bindings/{protocol,guards}.ts`、`fixtures/*.json`、ts-rs 依赖+全部 `#[ts]` 属性、导出/漂移测试退役；cargo tree 证 ts-rs 出图；样本 serde 往返覆盖由 `tests/surface_coverage.rs` 内存承载（零覆盖损失）；W1 的 fixtures 深比漂移门随面作废；OPEN-5 消解；§D.8 存退役记录 | ✓ 裁决执行 | — |
...
| 层 | 文件面 | 职责 | 门禁 |
| --- | --- | --- | --- |
| L1 视图层 | `views/*.rs`（17 文件） | 纯渲染 + 本地交互态；不触协议发送面、不持内核对象句柄 | `views_never_touch_wire_or_kernel_handles`（sends 恒 0；内核句柄**恒 0**——U7b 完成：context_rail 的最后 handle 退休，assert 收紧为 =0） |
| L2 编排层 | `workspace.rs`（生产 2.9k=struct+类型园+核心编排）+ 八子模块 `workspace/{attach/plan_review/composer/external/right_pane/chips/render/composer_render}`（簇1-8）+ `multiplexer.rs`（1.7k）| workspace=UI 编排（attach 生命周期/面板草稿/park/棘轮冻结面）；multiplexer=**唯一 wire 面**（gateway client：calls/notes/fold） | `no_new_files_touch_wire_or_store`（豁免表显式有限：workspace/multiplexer/slash_command/source_gates/client_store_handle/workspace\tests.rs）+ 棘轮预算表（SENDS 24/STORE_WRITE 7/STORE_GLOBAL 10/FACADE_WRITE 2） |
| L3 网关/内核层 | session-core（`agent_server.rs` 生产 3.2k + follow/journal_query/projections/translate）+ agent crate（thread/engine/store） | 会话与日志归服务端单写；桌面仅持 landing 镜像与 leaf | session-core 套件（91）+ journal/replay 门禁 + REPLAY_COVERAGE_KINDS |

测试面（U9a 提取）：`workspace/tests.rs`（2108 行）、`agent_server/tests.rs`（6911 行）——子模块仅在父 `#[cfg(test)] mod tests;` 下编译，`super` 语义与提取前逐一相同。
...
2. compat ClientNote 三件：`CreateSession`/`Submit`/`Steer`——U1-flush 已落地（parked flush 改道网关，前台 flush 此前已迁）。**C4a-2 验证经 review round 3 P0-6 普查更正**：submit 走 `ClientCall::Submit`✓、steer 桌面无 UI✓;但 **CreateSession 的「生产发送方已清零」不实**——multiplexer `open_or_create(reopen=false)` 即生产腿（workspace landing 与 attach 调用,发 compat note:客户端铸 id + follow 于 SessionCreated 应答后开）,与 `create_session` 的 v2 腿双路径并存（删除前置见 §K.7.3 该行）；webui 三处 `method:'submit'/'steer'/'createSession'` 均为 **v2 `request()` call**（method 字符串曾误判为 note 残余）。实际发送方仅 **vscode extension host**（sidebarProvider/participant 的 submit note、sessionManager 的 createSession note——client-chosen-id + awaitSession 流，迁移是行为级改动，超出「仅保持编译」授权）→ 三件服务端臂原为 vscode 保留——该项目已整体删除(仓库边界裁决),gate 解除,删除归仓内 C4 follow-up。`AppendUserMessage` 批量前置插入为**合法 v1 note**（不在三件内）：flush 替代的语义决策点仍开放（逐项 Submit 在 idle 线程上首条即开行、余条并第二轮,与现状单轮批量不等价——需批量词汇或接受两轮形状并入档）。
