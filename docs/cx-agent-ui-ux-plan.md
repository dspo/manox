...
- ✅ Phase 0：补充了 `docs/cx-agent-ux-rubric.md` 与 `scripts/ux/cx-agent-smoke.sh`。
- ✅ Phase 1：TUI 底部状态栏拆分为持久状态（provider/model/session/usage/follow）+ 短时事件（transient event）。
- ✅ Phase 2：timeline 增加 tool role 展示，tool lifecycle 文案统一为 started/args-streaming/finished。
- ✅ Phase 3：补齐 `/help /usage /tools /clear /retry /draft /reasoning /quit` 命令并提供 draft/retry 恢复路径。
- ✅ Phase 4：approval 支持 allow once / allow for turn / allow for session，并增加风险摘要文案。
- ✅ Phase 5：引入 `error-recovery` mode，失败后给出 retry/draft 恢复动作提示。
