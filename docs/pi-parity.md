...
| 能力 | manox 位置（规模） | pi 状态 | 说明 |
|---|---|---|---|
| 自动压缩 | compact.rs + thread.rs | 🟡 | 子件 a（token 计量）已落地（2026-07-30，见下行）；harness 已先行落地 TS Pi 触发层的压缩边界 timestamp guard，且边界经 `SessionTreeEntry::Compaction` 持久化、会话重建后 `recover_boundary()` 恢复（压缩前的 usage 不作锚点，回退纯启发式）；子件 b+c（摘要载体+核心压缩）、d（触发路由）待做，对齐 TS Pi compaction.ts |
