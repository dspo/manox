...
- `claude_md.rs` — 多级 CLAUDE.md 指令加载（Claude Code 兼容）。`load` 按 managed policy → `~/.claude/CLAUDE.md` + user rules → cwd 到文件系统根的祖先链（`CLAUDE.md` → `.claude/CLAUDE.md` → `.claude/rules/` → `CLAUDE.local.md`）收集指令文件；`@import` 四跳展开、HTML 块注释剥离、`claude_md_excludes` glob 排除、外部 import（cwd 与 `~/.claude` 之外）暂扣待审批。eager 块作为固定槽位 user message 注入 system 之后（per-request、不写历史、前缀缓存稳定）；lazy 层经 `discover_for_read` 在 `read_file` 成功时把子目录 CLAUDE.md 与 glob 命中的 scoped rules 以 `<system-reminder>` 追加到该 ToolResult 尾部（`lazy_instructions_injected` 去重，compaction 清空以便重读再注入）；内置 explore 子 agent 跳过注入。
...
- `tools/` — 15 内置工具（`read_file`/`write_file`/`edit_file`/`bash`/`grep`/`glob`/`list_directory`/`agent`/`ask_user`/`self_info`/`skill`/`worktree`/`monitor` + `enter_plan_mode`/`exit_plan_mode`）。写操作/bash 需审批；`AskUserQuestion` 走专属交互路径（`requires_user_input`，不经审批管线的任何环节）；`base_tools()` 返受限集（子 agent 基线），`default_registry()` 追加 MCP 工具（仅主 agent）。`enter_plan_mode`/`exit_plan_mode` 不进 registry，由 `build_completion_request` 按态合成（非 plan 态合成 `enter_plan_mode`，plan 态合成 `exit_plan_mode`），模型据此自主进入/退出 plan mode。
- `hashline/` — 行锚定补丁系统：`read_file` 输出 `[PATH#TAG]` + `N:TEXT`，`edit_file` 输入 `SWAP/DEL/INS` patch，tag 过期时 `try_recover` 做 3-way merge。`mcp/` — MCP 客户端（stdio + streamable HTTP），仅主 agent。`lsp/` + `crates/lsp` — rust-analyzer/gopls/pyright/typescript-language-server 的健康探测、懒启动、文档同步、语义查询与诊断；首次源码读取后台预热，成功 `Edit`/`Write` 后自动把有版本依据的文件诊断附加到工具结果，主/子 agent 均可用。
...
- **勿以善小而不为**：对于正面有效的 review 意见，即便其不构成阻塞，也应当尽量遵从。
- **Plan 应写入实施方式**：当制定涉及编码的 Plan 时，应当将`按 /gitwork:deliver 实施`写入 Plan 正文。
