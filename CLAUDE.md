...
- `claude_md.rs` — 多级 CLAUDE.md 指令加载（Claude Code 兼容）。`load` 按 managed policy → `~/.claude/CLAUDE.md` + user rules → cwd 到文件系统根的祖先链（`CLAUDE.md` → `.claude/CLAUDE.md` → `.claude/rules/` → `CLAUDE.local.md`）收集指令文件；`@import` 四跳展开、HTML 块注释剥离、`claude_md_excludes` glob 排除、外部 import（cwd 与 `~/.claude` 之外）暂扣待审批。eager 块作为固定槽位 user message 注入 system 之后（per-request、不写历史、前缀缓存稳定）；带 `paths` frontmatter 的 rules 进 `scoped` 待 lazy 层；内置 explore 子 agent 跳过注入。
