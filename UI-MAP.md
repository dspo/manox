...
Always-visible chip showing the thread's [CollaborationMode](#collaboration-modes) (`mode-chip-default` / `mode-chip-plan`). Click, `/plan`, or `shift-tab` (the `CycleCollaborationMode` action) cycles Plan↔Default. In Plan mode the tool set is read-only and the model submits a plan via a `<proposed_plan>` block surfaced as a [PlanReviewCard](#planreviewcard) drawer card (verdict buttons in the card, AskUserQuestion-style supplement label above the composer); in Default the full tool set is restored.
...
Plan-review drawer card rendered in the message list (`ConvItem::PlanReview { plan_text }`, `render_plan_review_card`), reusing the same drawer shell as [AskUserQuestion](#askuserquestion)'s card (`render_ask_user_card`): `pb_5().mb(px(-10.)).shadow_lg()` so the composer covers the card's tail and it reads as emerging from beneath the composer, a 180ms `ease_out_quint` slide-in animation, and a `PlanDrawer` key context. Triggered by `ThreadEvent::PlanReady` at a terminal stop in Plan mode — the `Workspace` PlanReady handler both pushes the plan body into the conversation as this card and sets `pending_plan_review` (no modal). Layout: an accent-tinted header (dashboard icon + "Plan" label) carrying three ghost icon buttons — download (clipboard fallback) / copy (clipboard) / open-in-side-panel (opens a [PlanPreviewTab](#planpreviewtab)); the plan body as a [Markdown](#markdown) view; and a top-bordered footer action row with the three verdicts — Stay-in-Plan (ghost, no-op) / Clear-&-Implement (ghost, exit Plan → Default, clear prior context, re-inject plan as the fresh-context seed) / Implement (primary, exit Plan → Default, seed milestones on [ContextRail](#contextrail), inject "Implement the approved plan." user turn) — all delegating to `respond_plan_review`. The composer stays live (see the supplement label below); a free-form send drops `pending_plan_review` so the card's verdict hides until a fresh `PlanReady` re-proposes, instead of acting on outdated text. The `<proposed_plan>` block content never enters the persisted message history; the card is a UI-only rendering of the in-memory plan text.

While `pending_plan_review` is set the composer mirrors the AskUserQuestion affordance: a muted `workspace-plan-supplement-label` line above the input ("Discuss or refine the plan, or pick a verdict above") and a `workspace-plan-supplement-placeholder` placeholder (`ComposerPlaceholderMode::Plan`), so the user may type to discuss or refine instead of picking a verdict. Replaces the former centered modal `PlanReviewOverlay`.

> Source: `agent-ui/src/views/message.rs` — `render_plan_review_card`. Composer affordance: `agent-ui/src/workspace.rs` — `render_composer` (`ComposerPlaceholderMode::Plan`, supplement label), `respond_plan_review`, `Workspace::send_user_turn` (pending-plan drop on free-form send). Conversation state: `ConversationState::push_plan_review`.
