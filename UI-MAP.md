...
- [TeamChip](#teamchip) · [TeamDrawer](#teamdrawer) · [PlanReviewDrawer](#planreviewdrawer)
...
A standalone tool-call card (`render_tool_call`) for the special-case tools that don't fold into a [ThinkingStatusRow](#thinkingstatusrow) batch — today `agent` sub-agent calls and `AskUserQuestion`. A model response's other tool calls batch into the `Thinking` container; their output renders via [TerminalPanel](#terminalpanel). A proposed plan now renders inline as a [PlanReviewCard](#planreviewcard) in the message list at turn end, with the verdict buttons in the [PlanReviewDrawer](#planreviewdrawer) above the composer.
...
Always-visible chip showing the thread's [CollaborationMode](#collaboration-modes) (`mode-chip-default` / `mode-chip-plan`). Click, `/plan`, or `shift-tab` (the `CycleCollaborationMode` action) cycles Plan↔Default. In Plan mode the tool set is read-only and the model submits a plan via a `<proposed_plan>` block surfaced as a [PlanReviewCard](#planreviewcard) (verdict via [PlanReviewDrawer](#planreviewdrawer)); in Default the full tool set is restored.
...
Inline plan-review item rendered in the message list (`ConvItem::PlanReview { plan_text }`, `render_plan_review_card`). Triggered by `ThreadEvent::PlanReady` at a terminal stop in Plan mode: the `Workspace` PlanReady handler both pushes the plan body into the conversation as this card and sets `pending_plan_review`, so the plan reads as an ordinary message and the verdict affordance lives in the [PlanReviewDrawer](#planreviewdrawer) above the composer (no modal). Bordered card: an accent-tinted header bar (dashboard icon + "Plan" label) carrying three ghost icon buttons — download (clipboard fallback) / copy (clipboard) / open-in-side-panel (opens a [PlanPreviewTab](#planpreviewtab)) — and a height-limited (max 300px) markdown body. The `<proposed_plan>` block content never enters the persisted message history; the card is a UI-only rendering of the in-memory plan text.

> Source: `agent-ui/src/views/message.rs` — `render_plan_review_card`. Conversation state: `ConversationState::push_plan_review`.

#### PlanReviewDrawer

Composer-anchored verdict drawer (`render_plan_review_drawer`), rendered in the footer `v_flex` above the composer while `pending_plan_review` is set. A bordered secondary-fill strip: dashboard icon + "Review Plan" title + a muted hint ("Discuss below, or choose an action") on the left, three buttons on the right — Stay-in-Plan (ghost, no-op, keep refining) / Clear-&-Implement (ghost, exit Plan → Default, clear prior context, re-inject plan as the fresh-context seed) / Implement (primary, exit Plan → Default, seed milestones on [ContextRail](#contextrail), inject "Implement the approved plan." user turn). All three delegate to `respond_plan_review`. The composer below stays live: the user may type a free-form message to discuss or refine the plan, in which case `pending_plan_review` is dropped on send (so the drawer hides until a fresh `PlanReady` re-proposes) and the turn re-runs in Plan mode. Replaces the former centered modal `PlanReviewOverlay`.

> Source: `agent-ui/src/workspace.rs` — `render_plan_review_drawer`, `respond_plan_review`, `Workspace::send_user_turn` (pending-plan drop on free-form send).
...
- **Milestone section** (collapsible via `ToggleCockpitTasks` / ctrl/cmd-shift-m, `cockpit_hide_tasks`): plan steps parsed from the approved `<proposed_plan>` block (seeded when the user picks Implement / Implement-clear-context on the [PlanReviewDrawer](#planreviewdrawer)). All `Pending` outside a turn; the first is promoted to `InProgress` while the thread runs, demoted back to `Pending` on terminal stop.
