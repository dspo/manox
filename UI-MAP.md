...
| Team | ✅ | 后端（Team/Member/TaskList 运行时 + 10 工具含 TeamDismiss/TeamStatus + 同伴消息路由 + 授权气泡 + member 停止生命周期通知）+ composer team chip（roster popover）+ MemberPanel 观察标签（右栏 Member tab：成员会话实时渲染 + 任务板 + header 最近停止原因 chip + dismiss 按钮） |
