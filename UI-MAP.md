...
Anchor-only virtual list backed by manox's `ChatList` (`manox-components::chat_list`, held via `ChatListState` on `Workspace`). The list caches no row heights — its only state is a single anchor (which row straddles the viewport top and by how much) plus a tail-follow flag — so every visible row is re-measured at a definite width each frame and no `splice`/`remeasure`/width-invalidation surface exists to go stale. `Bottom` alignment gives chat-log semantics (short histories sit at the viewport bottom, long ones scroll); tail-follow pins to the live end while following, disengages on an upward scroll, and re-arms when a scroll lands back at the bottom. Rows are keyed by `RowKey` (the `Entity<MessageItem>` id), so `scroll_to_row` survives insertions before the target. A per-row `ContentMask` clips any residual paint-overflow into its own slot. A floating "Jump to latest" button appears above the composer once the viewport has scrolled away from the bottom.

> Source: `agent-ui/src/workspace.rs` (`ChatListState` wiring), `components/src/chat_list.rs` (anchor-only layout)

#### MessageItem

Single rendered conversation item, centered, max-width 760px. Centering is `mx_auto` inside a **column** flex (`views::centered`), never `justify_center` in a row flex: a row flex takes its own height from the cross size its width-capped child reports at a probe width, so whenever the viewport is wider than the cap a width-sensitive body (a table with long cells is the extreme case) reports a far taller height than it paints, and the surplus stays in the row as a blank band. Pinned by `agent-ui/tests/chat_list_table_row.rs`. Each `MessageItem` renders one of the variant cards below based on `ConvItem` kind. Every kind that carries a text body — user, assistant, error, notice, team message, recap, retry detail, plan review — mounts a persistent `Entity<Markdown>` (`MessageItem::markdown`, created lazily by `ensure_markdown`) instead of rebuilding one per frame: a per-frame `Entity` resets the document's `DocSelection`/`FocusHandle` on every render and breaks drag-select + Cmd/Ctrl+C (the old `markdown_tv` fallback), while a persistent body keeps its selection state alive across frames and leaves inline links clickable.
