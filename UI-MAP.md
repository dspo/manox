...
Virtual list backed by gpui-component's `v_virtual_list` (`vlist(entity, list_state, render_item)`, `agent-ui/src/views/vlist.rs`). gpui-component owns the virtualization Element, scroll handle, visible-range computation, and content mask; manox owns the two concerns it leaves to the caller: per-item height measurement (each visible item is laid out at the list's definite content-mask width with `MinContent` available height inside the render closure, cached, and fed back as `item_sizes` next frame) and tail-follow arbitration (`FollowMode::Tail` re-pins to the live end while no upward scroll has disengaged it; re-arms when a scroll lands back at the bottom). Unmeasured items carry a small constant estimate (`ESTIMATED_ROW_H`) and self-correct the frame they enter the visible range; every visible row is re-measured every frame so a height that changed without a `remeasure` signal — async image load, font swap, lazy markdown reflow — self-corrects. Bottom alignment for short histories is a top spacer of `(viewport - content)` so a fresh chat logs against the composer. Count changes are reconciled via `splice` and in-place mutations via `remeasure_items`, both driven from the `ThreadEvent` handler's `ApplyOutcome`.

> Source: `agent-ui/src/views/vlist.rs` (state + `v_virtual_list` wrapper), `agent-ui/src/workspace.rs` (wiring)
