...
Index-anchored virtual list (`gpui::list(list_state, render_item)` with `ListAlignment::Bottom`, `ListSizingBehavior::Auto`, 2048px overdraw). The scroll position is a `ListOffset { item_ix, offset_in_item }` — an item index plus the pixel offset into that item — so a window-resize reflow re-measures every item and re-anchors **proportionally** (the reader's position within the scroll-top item is preserved), never stranding the viewport in empty space: this is the zed chat-panel anchoring model and the fix for the pixel-anchored container's "whole list goes blank on resize" failure. `FollowMode::Tail` re-pins to the end each layout while following (disengages on upward scroll, re-arms at the bottom); streaming growth of one item is reconciled via `remeasure_items` (Absolute anchor) and count changes via `splice`, both driven from the `ThreadEvent` handler's `ApplyOutcome`. Only the viewport + overdraw items render, so a long thread's first frame pays only for the visible turn.
