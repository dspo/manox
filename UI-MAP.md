...
| Team | ◐ 部分 | 成员经 `Steer(spawn="TeamMember")` 创建为真实 thread（sidebar 可见、可恢复）；同伴消息经 Steer Inject 路由。旧 roster 容器 `Entity<Team>` 与 6 个 roster 工具（TeamDismiss/TeamStatus 等）、授权冒泡、member 停止通知已退役/移除，member→parent 自主汇报未接线；MemberPanel 现为空壳（观察降级为打开 member thread 本身）。`Entity<Team>` 退役为后续清理项（见 PR Known degradations） |
